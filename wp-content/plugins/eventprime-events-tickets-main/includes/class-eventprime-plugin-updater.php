 ...
 *
 * @author EventPrime Plugin
 * @version 3.0.0
 */
class EventPrime_Plugin_Updater{
	 ...
	 */
	public function __construct( $_api_url, $_plugin_file, $_api_data = null,$key='eventprime' ) {

		global $ep_plugin_data;
                $this->key                      = $key;               
		...
		$this->failed_request_cache_key = $this->key.'_failed_http_' . md5( $this->api_url );

		$ep_plugin_data[ $this->slug ] = $this->api_data;

		/**
		 * Fires after the $ep_plugin_data is setup.
		 *
		 * @since x.x.x
		 *
		 * @param array $ep_plugin_data Array of EventPrime plugin data.
		 */
		do_action( 'post_'.$this->key.'_plugin_updater_setup', $ep_plugin_data );
	...
	public function show_update_notification( $file, $plugin ) {
		...
		if ( ! empty( $update_cache->response[ $this->name ]->sections->changelog ) ) {
			$changelog_link = add_query_arg(
				array(
					'edd_action'    => 'view_plugin_changelog',
		...
		printf(
			...
			esc_html__( 'There is a new version of %1$s available.', 'eventprime-zoom-meetings' ),
			esc_html( $plugin['Name'] )
		);

		if ( ! current_user_can( 'update_plugins' ) ) {
			echo ' ';
			esc_html_e( 'Contact your network administrator to install the update.', 'eventprime-zoom-meetings' );
		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
			echo ' ';
			printf(
				/* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
				__( '%1$sView version %2$s details%3$s.', 'eventprime-zoom-meetings' ),
			...
			echo ' ';
			printf(
				__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'eventprime-zoom-meetings' ),
				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
				esc_html( $update_cache->response[ $this->name ]->new_version ),
				'</a>',
			...
			printf(
				' %1$s%2$s%3$s',
				'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
				esc_html__( 'Update now.', 'eventprime-zoom-meetings' ),
				'</a>'
			);
		}
	...
	public function plugins_api_filter( $_data, $_action = '', $_args = null ) {
		...
		$ep_api_request_transient = $this->get_cached_version_info();

		//If we have no transient-saved value, run the API, set a fresh transient with the API value, and return that value too right now.
		if ( empty( $ep_api_request_transient ) ) {
			...
			$_data = $ep_api_request_transient;
		}
	 ...
	 *
	 * @see $this->key_Plugin_Updater::request_recently_failed
	 *
	 * @since 1.9.1
	 */
	...
	public function show_changelog() {
		...
		if ( empty( $_REQUEST['edd_action'] ) || 'view_plugin_changelog' !== $_REQUEST['edd_action'] ) {
			return;
		}
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'eventprime-zoom-meetings' ), esc_html__( 'Error', 'eventprime-zoom-meetings' ), array( 'response' => 403 ) );
		}
	...
	private function get_version_from_remote() {
		 ...
		 */
		$api_params = apply_filters( $this->key.'_plugin_updater_api_params', $api_params, $this->api_data, $this->plugin_file );
	...
	public function set_version_info_cache( $value = '', $cache_key = '' ) {
		...
		delete_option( 'ep_api_request_' . md5( serialize( $this->slug . $this->api_data['license'] . $this->beta ) ) );
	}
	...
	private function verify_ssl() {
		return (bool) apply_filters( $this->key.'_api_request_verify_ssl', true, $this );
	}
	...
	private function get_cache_key() {
		...
		return $this->key.'_' . md5( serialize( $string ) );
	}
