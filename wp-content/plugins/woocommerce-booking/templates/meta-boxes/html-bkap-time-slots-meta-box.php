	...
	foreach ( $booking_settings['booking_time_settings'] as $bkap_weekday_key => $bkap_weekday_value ) {
		...
		foreach ( $bkap_weekday_value as $day_key => $time_data ) {
			...
			if ( $number >= $bkap_start_record_from && $number <= $bkap_end_record_on ) {
				...
				<tr id="bkap_date_time_row_<?php echo esc_attr( $number ); ?>" class="bkap_dateday_td">
					<td width="20%">
						<select id="bkap_dateday_selector_<?php echo esc_attr( $number ); ?>" class="bkap_dateday_selector" multiple="multiple" disabled >
							...
							foreach ( $bkap_weekdays as $w_value => $w_name ) {
								...
								if ( $w_value == $bkap_weekday_key ) {
									...
									printf( "<option value='%s' %s>%s</option>\n", esc_attr( $w_value ), esc_attr( $bkap_selected ), esc_html( $w_name ) );
							...
							foreach ( $specific_dates as $dates => $lockout ) {
								...
								printf( "<option value='%s' %s>%s</option>\n", esc_attr( $dates ), esc_attr( $bkap_selected ), esc_html( $dates ) );
					...
					<td width="10%" class="bkap_from_time_td"><input id="bkap_from_time_<?php echo esc_attr( $number ); ?>" type="text" name="quantity" style="width:100%;" pattern="^([0-1][0-9]|[2][0-3]):([0-5][0-9])$" title="Please enter time in 24 hour format e.g 14:00 or 03:00" placeholder="HH:MM" minlength="5" maxlength="5" onkeypress="return bkap_isNumberKey(event)" value="<?php echo esc_attr( $bkap_from_time ); ?>" readonly></td>
					<td width="10%" class="bkap_to_time_td"><input id="bkap_to_time_<?php echo esc_attr( $number ); ?>" type="text" name="quantity" style="width:100%;" pattern="^([0-1][0-9]|[2][0-3]):([0-5][0-9])$" title="Please enter time in 24 hour format e.g 14:00 or 03:00" placeholder="HH:MM" minlength="5" maxlength="5" onkeypress="return bkap_isNumberKey(event)" value="<?php echo esc_attr( $bkap_to_time ); ?>" readonly></td>
					<td width="10%" class="bkap_lockout_time_td"><input id="bkap_lockout_time_<?php echo esc_attr( $number ); ?>" onkeypress="return bkap_only_number( event )" type="number" name="quantity" style="width:100%;" min="0" placeholder="<?php echo esc_html__( 'Max bookings', 'woocommerce-booking' ); ?>" value="<?php echo esc_attr( $bkap_lockout ); ?>" class = "bkap_default" ></td>
					<td width="10%" class="bkap_price_time_td"><input id="bkap_price_time_<?php echo esc_attr( $number ); ?>" type="text" name="quantity" style="width:100%;" placeholder="Price" value="<?php echo esc_attr( $bkap_price ); ?>" class = "bkap_default wc_input_price"></td>
					<td width="10%" style="text-align:center;" class="bkap_global_time_td">
						<label class="bkap_switch">
							<input id="bkap_global_time_<?php echo esc_attr( $number ); ?>" type="checkbox" name="bkap_global_timeslot" style="margin-left: 35%;" <?php echo esc_attr( $bkap_global_checked ); ?> class = "bkap_default">
							<div class="bkap_slider round"></div>
						</label>
					</td>
					<td width="23%" class="bkap_note_time_td"><textarea id="bkap_note_time_<?php echo esc_attr( $number ); ?>" rows="1" cols="2" style="width:100%;" class = "bkap_default" ><?php echo esc_html( $bkap_note ); ?></textarea></td>
					<td width="4%"  class="bkap_actions" id="bkap_actions_<?php echo esc_attr( $number ); ?>" style="text-align: center;cursor:pointer;">
						<a href="#" id="bkap_edit_<?php echo esc_attr( $number ); ?>" class="bkap_edit_row" ><i class="fa fa-edit" aria-hidden="true"></i></a>
						<a href="#" id="bkap_close_<?php echo esc_attr( $number ); ?>" class="bkap_delete_row" ><i class="fa fa-trash" aria-hidden="true"></i></a>
					</td>
				</tr>
