...
<div id="bkap_edit_modal_<?php echo esc_attr( $bkap_cart_item_key ); ?>" class="bkap-modal">
	...
	<div class="bkap-booking-contents">
		...
		<div class="bkap-booking-header">
			...
			if ( ! empty( $bkap_order_id ) ) {
				$bkaporder         = wc_get_order( $bkap_order_id );
				$is_coupon_applied = count( $bkaporder->get_coupon_codes() );
			} else {
				$is_coupon_applied = 0;
			}
			$edit_booking_text = ( Bkap_Cancel_Booking::is_booking_page() || is_wc_endpoint_url( 'view-order' ) ) ? 'Reschedule Bookings' : 'Edit Bookings';
			?>
			<div class="bkap-header-title">
				<h1 class="product_title entry-title">
					<?php $edit_booking_label = apply_filters( 'bkap_edit_booking_label', __( $edit_booking_text, 'woocommerce-booking' ) ); ?>
					<?php echo wp_kses_post( $product_obj->get_name() . ' - ' . $edit_booking_label ); ?>
				</h1>
				<?php
				if ( $is_coupon_applied ) {
					...
					echo '<p class="bkap-reschedule-notification">' . esc_html( $reschedule_notification ) . '</p>';
				}
				?>
			</div>
			<div class="bkap-header-close" onclick='bkap_edit_booking_class.bkap_close_popup(<?php echo esc_attr( $product_id ); ?>, "<?php echo esc_attr( $bkap_cart_item_key ); ?>")'>&times;</div>

		</div>

		<div style="clear: both;"></div>

		<div id="modal-body-<?php echo esc_attr( $bkap_cart_item_key ); ?>" class="modal-body">

			<?php

			BKAP_Scripts::include_frontend_scripts_js( $product_id );
			BKAP_Scripts::inlcude_frontend_scripts_css( $product_id );
			...
			if ( isset( $bkap_booking['hidden_date'] ) && '' !== $bkap_booking['hidden_date'] ) {
				$hidden_dates['hidden_date'] = gmdate( 'j-n-Y', strtotime( $bkap_booking['hidden_date'] ) );
			} else {
				$hidden_dates['hidden_date'] = gmdate( 'j-n-Y' );
			}

			$hidden_dates['hidden_checkout'] = '';
			if ( isset( $bkap_booking['hidden_date_checkout'] ) && '' !== $bkap_booking['hidden_date_checkout'] ) {
				$hidden_dates['hidden_checkout'] = gmdate( 'j-n-Y', strtotime( $bkap_booking['hidden_date_checkout'] ) );
			}
			...
			?>
			<input type="hidden" class="variation_id" value="<?php echo esc_attr( $variation_id ); ?>" />
			...
			if ( isset( $bkap_booking['resource_id'] ) ) :
				...
				<div class="resource_id_container">
					<input type="hidden" name="chosen_resource_id" id="chosen_resource_id" class="rform_hidden" value="<?php echo esc_attr( $resource_id ); ?>">
				</div>
			...
			<?php if ( isset( $bkap_booking['persons'] ) ) : ?>
				<div class="persons_container">
					...
					if ( isset( $bkap_booking['persons'][0] ) ) {
						?>
						<input type="hidden" name="chosen_person" id="chosen_person" class="rform_hidden" value="<?php echo esc_attr( $bkap_booking['persons'][0] ); ?>">
						<?php
					} else {
						foreach ( $bkap_booking['persons'] as $p_key => $p_value ) {
							?>
							<input type="hidden" name="chosen_person_<?php echo esc_attr( $p_key ); ?>" id="chosen_person_<?php echo esc_attr( $p_key ); ?>" class="rform_hidden" value="<?php echo esc_attr( $p_value ); ?>">
							<?php
						}
					}
					?>
			...
			<?php if ( isset( $bkap_booking['fixed_block'] ) && $bkap_booking['fixed_block'] != '' ) : ?>
				...
				<div class="fixed_block_container">
					<input type="hidden" name="chosen_fixed_block" id="chosen_fixed_block" class="rform_hidden" value="<?php echo esc_attr( $bkap_booking['fixed_block'] ); ?>">
				</div>
			...
			<?php if ( isset( $bkap_addon_data['gf_options'] ) && $bkap_addon_data['gf_options'] !== '' ) : ?>
				...
				<div class="ginput_container_total">
					<input type="hidden" name="gravity_forms_options" id="gravity_forms_options" class="gform_hidden" value="<?php echo esc_attr( $bkap_addon_data['gf_options'] ); ?>">
				</div>
			...
			<?php if ( isset( $bkap_addon_data['wpa_options'] ) && $bkap_addon_data['wpa_options'] !== '' ) : ?>
				...
				<div id="product-addons-total" data-show-grand-total="1" data-type="simple" data-price="" data-raw-price="" data-addons-price="<?php echo esc_attr( $bkap_addon_data['wpa_options'] ); ?>"></div>
			...
			<div class="measurement_needed_section">
				<input type="hidden" name="_measurement_needed" id="_measurement_needed" value="<?php echo esc_attr( $bkap_addon_data['_measurement_needed'] ); ?>">
			</div>
		...
		<div class="modal-footer">
			...
			<input
				...
				id="confirm_bookings_<?php echo esc_attr( $bkap_cart_item_key ); ?>"
				onclick='bkap_edit_booking_class.bkap_confirm_booking(<?php echo esc_attr( $product_id ); ?>, "<?php echo esc_attr( $bkap_cart_item_key ); ?>")'
			...
			<input
				...
				name="cancel_modal"
				id="cancel_modal"
				onclick='bkap_edit_booking_class.bkap_close_popup(<?php echo esc_attr( $product_id ); ?>, "<?php echo esc_attr( $bkap_cart_item_key ); ?>")'
				value="<?php esc_html_e( 'Cancel', 'woocommerce-booking' ); ?>"
				class="bkap_modal_button_class"
			/>
		</div>
	</div>
