...
class bkap_common {
	...
	public static function bkap_check_timeslot_for_weekday( $product_id, $start_date, $booking_settings ) {
		...
		$start_weekday         = gmdate( 'w', strtotime( $start_date ) );
		...
		array_key_exists( gmdate( 'j-n-Y', strtotime( $start_date ) ), $booking_settings['booking_time_settings'] ) ) {
			return true;
		}
	...
	public static function bkap_get_all_timeslots_for_weekday_date( $product_id, $start_date, $booking_settings ) {
		...
		if ( isset( $booking_settings['booking_time_settings'] ) && is_array( $booking_settings['booking_time_settings'] ) ) {
			...
			if ( isset( $booking_settings['booking_time_settings'][ $start_booking_weekday ] ) ) {
				...
				$start_date_jny = gmdate( 'j-n-Y', strtotime( $start_date ) );
	...
	public static function bkap_get_betweendays( $start_date, $end_date, $format = 'd-n-Y', $recurring_days = array() ) {
		...
		if ( $start_date_timestamp !== $end_date_timestamp ) {
			while ( $start_date_timestamp < $end_date_timestamp ) {
				...
				if ( $recurring_passed ) {
					$weekday         = gmdate( 'w', strtotime( $start_date ) );
				...
				if ( $consider_date || empty( $recurring_days ) ) {
					$days[] = gmdate( $format, strtotime( $start_date ) );
				}
				$start_date           = gmdate( $format, strtotime( '+1 day', strtotime( $start_date ) ) );
	...
	public static function bkap_get_betweendays_when_flat( $StartDate, $EndDate, $pro_id, $format = 'd-n-Y' ) {
		...
		if ( $CurrentDate_timestamp != $EndDate_timestamp ) {
			...
			while ( $CurrentDate_timestamp < $EndDate_timestamp ) {
				$CurrentDate           = gmdate( $format, strtotime( '+1 day', strtotime( $CurrentDate ) ) );
	...
	/**
	 * Return Variation ID based on selected language.
	 *
	 ...
	 * @version 6.4.0
	 */
	 public static function bkap_get_variation_id_based_on_selected_lang( $variation_id ) {

		if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {
			$current_language = apply_filters( 'wpml_current_language', null );
			$variation_id     = apply_filters( 'wpml_object_id', $variation_id, 'product_variation', false, $current_language );
		}
	...
	public static function bkap_get_price( $product_id, $variation_id, $product_type, $check_in = '', $check_out = '' ) {
		...
		if ( $wpml_multicurreny_enabled == 'no' ) {
			...
			// Fix seasonal addon #124: Compatibility with woodmart theme.
			$price = self::bkap_get_price_woodmart($price, $product_id, $product_obj );
			...
			foreach ( $get_bundle_items as $b_key => $b_value ) {
				...
				if ( isset( $price_results ) && count( $price_results ) > 0 ) {
					...
					if ( 'yes' == $price_type ) {
						...
						$booking_type = bkap_type( $bundle_child_id );
		...
		if ( ! empty( $composite_data ) ) {
			...
			foreach ( $composite_data as $c_key => $c_value ) {
				...
				if ( isset( $c_value['p_id'] ) && '' !== $c_value['p_id'] ) {
					...
					$is_bookable   = self::bkap_get_bookable_status( $c_value['p_id'] );
					...
					if ( isset( $component_data['priced_individually'] ) && 'yes' === $component_data['priced_individually'] ) {
						...
						if ( $is_bookable ) {
							// phpcs:disable
							if ( 'multiple_days' === $booking_type && isset( $_POST['checkin_date'] ) ) {
								$param = array(
									gmdate( 'Y-m-d', strtotime( $_POST['checkin_date'] ) ),
									gmdate( 'Y-m-d', strtotime( $_POST['current_date'] ) ),
									$booking_type,
								);
							} elseif ( isset( $_POST['bkap_date'] ) ) {
								$param = array(
									gmdate( 'Y-m-d', strtotime( $_POST['bkap_date'] ) ),
									gmdate( 'w', strtotime( $_POST['bkap_date'] ) ),
									$booking_type,
								);
							}
							// phpcs:enable
					...
					if ( $quantity > 1 && $child_type != 'bundle' ) {
						$child_price = $child_price * sanitize_text_field( wp_unslash( $_POST['quantity'] ) );
					}

					if ( $diff_days > 1 && $child_type != 'bundle' ) {
						$child_price = $child_price * sanitize_text_field( wp_unslash( $_POST['diff_days'] ) );
					}
	...
	public static function woo_product_addons_compatibility_cart( $diff_days, $cart_item_meta, $product_quantity ) {
		...
		if ( class_exists( 'WC_Product_Addons' ) ) {
			...
			if ( isset( $cart_item_meta['addons'] ) && isset( $_POST['total_price_calculated'] ) ) {
				...
				$price          = (float) $_POST['total_price_calculated'] / $product_quantity;
				$wpaprice       = array();
		...
		return apply_filters( 'modify_bkap_addon_price', $addons_price, $diff_days, $cart_item_meta, $product_quantity );
	}
	...
	public static function bkap_get_number_of_slots( $product_id, $date_check_in ) {
		...
		if ( is_array( $booking_settings['booking_time_settings'] ) && count( $booking_settings['booking_time_settings'] ) > 0 ) {
			if ( array_key_exists( $date_check_in, $booking_settings['booking_time_settings'] ) ) {
				...
				$weekday         = gmdate( 'w', strtotime( $date_check_in ) );
				$booking_weekday = 'booking_weekday_' . $weekday;
	...
	public static function bkap_update_order_item_meta( $item_id, $product_id, $booking_data, $gcal_import = false, $add_item_meta = false, $item = array() ) {
		...
		if ( isset( $booking_data['hidden_date'] ) && '' !== $booking_data['hidden_date'] ) {
			...
			if ( ! $add_item_meta ) {
				...
				self::save_booking_information_to_order_note( $item_id, $order_id, $name . ': ' . sanitize_text_field( $date_select, true ) );
			}
		...
		if ( isset( $booking_data['time_slot'] ) && $booking_data['time_slot'] != '' ) {
			...
			foreach ( $exploded_time as $key => $value ) {
				...
				if ( $time_format == '12' ) {
					$from_time = gmdate( 'h:i A', strtotime( $time_exploded[0] ) );
					$to_time   = isset( $time_exploded[1] ) ? gmdate( 'h:i A', strtotime( $time_exploded[1] ) ) : '';
				}
			...
			if ( ! $add_item_meta ) {
				...
				self::save_booking_information_to_order_note( $item_id, $order_id, $name_time_slot . ': %s' . $time_slot_to_display );
			}
		}
		...
		if ( isset( $booking_data['persons'] ) && $booking_data['persons'] ) {
			if ( isset( $booking_data['persons'][0] ) ) {
				if ( $add_item_meta ) {
					$item->add_meta_data( BKAP_Person::bkap_get_person_label( $product_id ), $booking_data['persons'][0] );
				} else {
					wc_add_order_item_meta( $item_id, BKAP_Person::bkap_get_person_label( $product_id ), $booking_data['persons'][0] );
				}
			} else {
		...
		if ( isset( $booking_data['selected_duration'] ) && $booking_data['selected_duration'] != 0 ) {
			...
			if ( ! $add_item_meta ) {
				// Save Booking Time Information to Order Note.
				self::save_booking_information_to_order_note( $item_id, $order_id, $time_slot_label . ': ' . $time_slot );
			}
		}
	...
	public static function bkap_add_hour_to_date( $start, $time, $hour, $product_id, $d_type = '' ) {
		...
		// $end_date = gmdate( 'YmdHis', $hidden_end_with_time );
	...
	public static function bkap_add_notice() {
		// phpcs:disable WordPress.Security.NonceVerification
		$product_id = isset( $_POST['post_id'] ) ? sanitize_text_field( wp_unslash( $_POST['post_id'] ) ) : '';
	...
	public static function get_bkap_translated_status_label( $status_label ) {
		...
		switch ( $status_label ) {
			...
			case 'Cancelled':
				...
				break;
			default:
				$translated_label = __( $status_label, 'woocommerce-booking' ); // phpcs:ignore
				break;
		}
	...
	public static function bkap_check_specific_date_has_timeslot( $product_id ) {
		...
		$current_time           = current_time( 'timestamp' );
		$day                    = strtotime( gmdate( 'Y-m-d', $current_time ) );
					...
					if ( in_array( $booking_specific_dates_key, $booking_time_settings_key ) ) {
						...
						foreach ( $time_data as $key => $value ) {
							if ( '' != $value['from_slot_hrs'] ) {
								...
								if ( strtotime( $booking_specific_dates_key . ' ' . $time ) > strtotime( gmdate( 'Y-m-d H:s', $current_time ) ) ) {
									return true;
								}
							}
						}
	...
	public static function bkap_get_order_item_addon_data( $order_item ) {
		...
		if ( isset( $order_item['Total'] ) && $order_item['Total'] !== '' ) {
			$addon_pricing_data['gf_options'] = str_replace( html_entity_decode( $currency_symbol, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401, 'UTF-8' ), '', $order_item['Total'] );
		}
	...
	public static function bkap_bundle_add_to_cart_config( $product, $product_id ) {
	...
	public static function get_booking_ids_from_order_id( $order_id ) {
		...
		$booking_ids = array();
		if ( absint( $order_id ) > 0 && wc_get_order( $order_id ) ) {
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL
			$results = $wpdb->get_results( $wpdb->prepare( 'SELECT ID from `' . $wpdb->prefix . 'posts` WHERE post_parent = %d && post_type = %s', $order_id, 'bkap_booking' ) );
	...
	public static function bkap_get_bookings( $post_status, $additional_args = array() ) {
		...
		$search         = ( isset( $_GET['s'] ) && '' !== $_GET['s'] ) ? sanitize_text_field( wp_unslash( $_GET['s'] ) ) : '';
		$date           = ( isset( $_GET['m'] ) && '' !== $_GET['m'] ) ? sanitize_text_field( wp_unslash( $_GET['m'] ) ) : '';
	...
	public static function bkap_get_date_format( $global_settings = array() ) {
		...
		$date_formats = self::get_booking_global_value( 'bkap_date_formats' );
	...
	public static function bkap_date_from_session_cookie( $date ) {
		...
		if ( isset( WC()->session )
			...
			$sdate = sanitize_text_field( wp_unslash( $_COOKIE[ $date ] ) );
		} else {
			$sdate = false;
		}

		return apply_filters( 'bkap_date_from_session_cookie', $sdate ) ;
	}
	...
	public static function create_zoom_meetings_for_edited_bookings( $order_id, $booking_id ) {
		...
		foreach ( $new_booking_data as $data ) {
			$updated_meeting_data = Bkap_Zoom_Meetings::bkap_create_zoom_meeting( $booking_id, $data, 'update' );
	...
	/**
	 * Gets the value for a specified booking global variable.
	 *
	 * @param string $variable Booking Global variable to be returned.
	 * @return array
	 * @since 5.19.0
	 */
	public static function get_booking_global_value( $variable ) {
		return isset( $GLOBALS[ $variable ] ) ? $GLOBALS[ $variable ] : array();
	}

	/**
	 * Function to return the Booking Reminders templates information.
	 *
	 * @return Array Array of Reminder ID - Title information.
	 *
	 * @since 6.4.0
	 */
	public static function bkap_get_email_reminders() {
		...
		if ( count( $all_reminders ) > 0 ) {
			...
			foreach ( $all_reminders as $key => $value ) {
				...
				$reminders[ $reminder_id ] = $value->post_title;
			}
		}
	...
	/**
	 * Gets the discounted price for a products based on quantity selected when woodmart theme is active.
	 *
	 * @param string $price Booking price.
	 * @param string $product_id Product id.
	 * @param string $product Product object.
	 * @return array
	 * @since 5.19.0
	 */
	public static function bkap_get_price_woodmart( $price, $product_id, $product ) {

		if ( ! class_exists( '\XTS\Modules\Dynamic_Discounts\Manager' ) ) {
			return $price; // Exit if the Manager class is not available.
		}
		if ( ! $product ) {
			return $price;
		}

		$qty = isset( $_POST['quantity'] ) ? (int) $_POST['quantity'] : 1;//phpcs:ignore WordPress.Security.NonceVerification

		$manager  = \XTS\Modules\Dynamic_Discounts\Manager::get_instance();
		$discount = $manager->get_discount_rules( $product );

		if ( empty( $discount['discount_rules'] ) || $discount['_woodmart_rule_type'] !== 'bulk' ) {
			return $price;
		}

		foreach ( $discount['discount_rules'] as $rule ) {
			$from = (int) $rule['_woodmart_discount_rules_from'];
			$to   =  '' !== $rule['_woodmart_discount_rules_to'] ? (int) $rule['_woodmart_discount_rules_to'] : PHP_INT_MAX;

			if ( $qty >= $from && $qty <= $to ) {
				$type  = $rule['_woodmart_discount_type'];
				$value = 'amount' === $type ? apply_filters( 'woodmart_product_pricing_amount_discounts_value', $rule['_woodmart_discount_amount_value'] ) : $rule['_woodmart_discount_percentage_value'];

				$discounted_price = \XTS\Modules\Dynamic_Discounts\Main::get_instance()->get_product_price(
					$price,
					array(
						'type'  => $type,
						'value' => $value,
				...
				if ( wc_tax_enabled() ) {
					$discounted_price = ( 'incl' === get_option( 'woocommerce_tax_display_shop' ) )
						? wc_get_price_including_tax( $product, [ 'price' => $discounted_price ] )
						: wc_get_price_excluding_tax( $product, [ 'price' => $discounted_price ] );
				}
