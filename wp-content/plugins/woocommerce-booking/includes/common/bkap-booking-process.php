...
if ( ! class_exists( 'bkap_booking_process' ) ) {
	...
	class bkap_booking_process {
		...
		public function __construct() {
			...
			add_action( 'wp_enqueue_scripts', array( &$this, 'bkap_dequeue_conflicting_styles_scripts' ), 9999999 );
		}

		/**
		 * Dequeueing the conflicting style/script on the front end.
		 *
		 * @since 6.3.0
		 */
		public static function bkap_dequeue_conflicting_styles_scripts() {

			if ( is_product() || is_cart() || is_checkout() ) {

				if ( is_plugin_active( 'swpm-form-builder/swpm-form-builder.php' ) ) {
					wp_dequeue_style( 'swpm-jqueryui-css' );
				}

				$active_theme      = wp_get_theme();
				$active_theme_name = array( "Bricks", "Bricks Child Theme" );

				if ( ( ( is_plugin_active( 'fusion-core/fusion-core.php' ) && is_plugin_active( 'fusion-builder/fusion-builder.php' ) ) || ( in_array( $active_theme->name, $active_theme_name ) || in_array( $active_theme->parent_theme, $active_theme_name ) ) ) && class_exists( 'BKAP_Files' ) ) {


					BKAP_Scripts::bkap_front_side_scripts_js();
					BKAP_Scripts::bkap_front_side_scripts_css();
					wp_register_script( 'booking-process', BKAP_Files::rewrite_asset_url( '/assets/js/booking-process.js', BKAP_FILE ), '', BKAP_VERSION, false );
		 ...
		 * @since 4.1.0
		 */
		public static function bkap_localize_process_script( $post_id, $edit = false, $is_called_from_vue = false ) {
			...
			$dateymd            = gmdate( 'Y-m-d', $current_time );
				...
				foreach ( $recurring_date_array as $wkey => $wval ) {
					...
					if ( $default == 'Y' ) {
						...
						if ( $booking_settings['booking_recurring_booking'] == 'on' ) {
							...
							if ( isset( $wval ) && $wval == 'on' && $check_time && isset( $booking_settings['booking_time_settings'] ) && array_key_exists( $wkey, $booking_settings['booking_time_settings'] ) && count( $booking_settings['booking_time_settings'][ $wkey ] ) > 0 ) {
			...
			// Current Page
			$bkap_page                               = $is_called_from_vue ? 'bkap_post' : bkap_get_page();
				...
				if ( isset( $booking_settings['booking_enable_multiple_time'] ) && $booking_settings['booking_enable_multiple_time'] == 'multiple' ) {
					...
					if ( strpos( $url, $template_name ) !== false ) {
						...
						foreach ( $view_order->get_items() as $item ) {
							...
							$_wapbk_booking_date = gmdate( 'j-n-Y', strtotime( wc_get_order_item_meta( $itm_id, '_wapbk_booking_date' ) ) );
			...
			foreach ( $individual_holidays as $date => $years ) { // array format [date] => years to recur
				...
				if ( $years > 0 ) {
					for ( $i = 1; $i <= $years; $i++ ) {
						...
						$holiday_array[] = gmdate( 'j-n-Y', strtotime( '+' . $i . 'years', strtotime( $date ) ) );
					}
				}
			}
			...
			if ( is_array( $holiday_ranges ) && count( $holiday_ranges ) > 0 ) {
				...
				foreach ( $holiday_ranges as $ranges ) {
					...
					foreach ( $days_in_between as $dates ) {
						...
						$holiday_array[] = gmdate( 'j-n-Y', strtotime( $dates ) ); // add each date

						// if recurring years is greater than 0
						if ( $recur > 0 ) {
							for ( $i = 1; $i <= $recur; $i++ ) {
								// add the date for the future years
								$holiday_array[] = gmdate( 'j-n-Y', strtotime( '+' . $i . 'years', strtotime( $dates ) ) );
							}
						}
					}
				}
			...
			if ( isset( $booking_settings['booking_specific_booking'] ) && $booking_settings['booking_specific_booking'] == 'on' ) {
				...
				if ( ! empty( $booking_dates_arr ) ) {
					$booking_dates_array_keys = array_keys( $booking_dates_arr );
					usort(
						$booking_dates_array_keys,
						function ( $a, $b ) {
							return strtotime( $a ) - strtotime( $b );
						}
					);
					// @since 4.0.0 they are now saved as date (key) and lockout (value).
					foreach ( $booking_dates_array_keys as $k => $v ) {
						$booking_dates_str .= '"' . $v . '",';
					}
				}
			...
			if ( $specific_booking && is_array( $custom_ranges ) && count( $custom_ranges ) > 0 ) {
				...
				foreach ( $custom_ranges as $range ) {
					...
					for ( $i = 0; $i <= $recur; $i++ ) {
						...
						$start_date = gmdate( 'j-n-Y', strtotime( "+$i years", strtotime( $start ) ) );
						$end_date   = gmdate( 'j-n-Y', strtotime( "+$i years", strtotime( $end ) ) );
						$ranges[]   = array(
							'start' => $start_date,
							'end'   => $end_date,
						);
			...
			if ( isset( $booking_settings['booking_enable_time'] ) && ( 'on' === $booking_settings['booking_enable_time'] || 'dates_time' === $booking_settings['booking_enable_time'] ) ) {
				...
				if ( isset( $booking_settings['booking_time_settings'] ) ) {
					if ( is_array( $booking_settings['booking_time_settings'] ) && array_key_exists( $min_date, $booking_settings['booking_time_settings'] ) ) {
						...
						$weekday         = gmdate( 'w', strtotime( $min_date ) );
				...
				if ( $last_slot_hrs == 0 && $last_slot_min == 0 ) {
					...
					$booking_date2 = gmdate( 'Y-m-d H:i', strtotime( $booking_date2 ) );

					$date2         = new DateTime( $booking_date2 );
					$booking_date1 = gmdate( 'Y-m-d H:i', $current_time );
					...
					if ( ! $include ) {
						$min_date = gmdate( 'j-n-Y', strtotime( $min_date . '+1 day' ) );
					}
				}
			...
			if ( isset( $booking_settings['booking_enable_time'] ) && in_array( $booking_settings['booking_enable_time'], array( 'on', 'dates_time' ) ) || ( in_array( $booking_settings['booking_enable_time'], array( 'on', 'dates_time' ) ) && $booking_settings['booking_specific_booking'] == 'on' ) ) { // if date_time booking method
				...
				foreach ( $single_day_lockedout_dates as $k => $v ) {
					$lockout_temp         = gmdate( 'j-n-Y', strtotime( $v ) );
			...
			$lockout_dates = is_admin() && isset( $_POST['create_booking_post_data'] ) && isset( $_POST
				['create_booking_post_data']['show_disabled_dates'] ) && 'on' === $_POST
				['create_booking_post_data']['show_disabled_dates'] ? '' : $lockout_dates;
			...
			if ( 'multiple_days' === $booking_type ) {
				...
				$bkap_get_booked = bkap_get_booked( $product_id );

				$booked_dates = '';
				if ( ! empty( $bkap_get_booked['booked_dates'] ) ) {
					$booked_dates = implode( '","', $bkap_get_booked['booked_dates'] );
					$booked_dates = '"' . $booked_dates . '"';
					$lockout_dates_array = $bkap_get_booked['booked_dates'];
				}

				if ( ! empty( $bkap_get_booked['booked_enddates'] ) ) {
					$booked_enddates = implode( '","', $bkap_get_booked['booked_enddates'] );
					$booked_enddates = '"' . $booked_enddates . '"';
				}

				$additional_data['wapbk_hidden_booked_dates']          = $booked_dates;
				$additional_data['wapbk_hidden_booked_dates_checkout'] = $booked_enddates;
			}

			$current_time = strtotime( $min_date );
			$default_date = '';
			$fix_min_day  = gmdate( 'w', strtotime( $min_date ) );
			...
			if ( $default_date == '' ) {
				$no_default_found = 0;
				$default_date     = gmdate( 'j-n-Y', current_time( 'timestamp' ) );
			}
			...
			|| ( isset( $booking_settings['bkap_all_data_unavailable'] ) && 'on' === $booking_settings['bkap_all_data_unavailable'] )
				...
				$manage_time_availability = apply_filters( 'bkap_manage_time_availability_settings', $booking_settings['bkap_manage_time_availability'], $booking_settings, $product_id );
				...
				$thirtyth_date                = gmdate( 'j-n-Y', strtotime( '+30 day', $last_datein_adic_str ) );
						...
						if ( ! $date_checked && $bkap_availabile_date_str >= strtotime( $date_range_start ) && $bkap_availabile_date_str <= strtotime( $date_range_end ) ) {
							...
							if ( $value['bookable'] == 0 && ! in_array( $a_value, $mta_must_array ) ) {
								if ( $default_date == $a_value ) {
									$default_date = gmdate( 'j-n-Y', strtotime( $default_date . ' +1 day' ) );
								}
								array_push( $mta_holiday_array, $a_value );
							}
							$holiday_check = true;
			...
			$resource_disabled_time_slots = array();
			if ( ! empty( $resource_array ) ) {
				...
				if ( strpos( $days, '-' ) == false ) {
					...
					$start_booking_str            = strtotime( $default_date );
					$max_booking_date             = apply_filters( 'bkap_max_date', gmdate( 'Y-m-d', $start_booking_str ), $days, $booking_settings );
				...
				foreach ( $resource_ids as $resource_id ) {
					...
					if ( ! isset( $bkap_all_data_unavailable ) && is_array( $resource_availability ) && count( $resource_availability ) > 0 ) {
						...
						$resource_disabled_time_slots[ $resource_id ] = array();
								...
								switch ( $value['type'] ) {
									...
									case 'time:6':
										...
										if ( $check ) {
											...
											// Issue #4538.
											if ( ! empty( $timeslots_for_date ) && 0 == $value['bookable'] ) {
												if ( 'time' === $value['type'] ) {
												...
												foreach ( $timeslots_for_date as $timeslot_for_date ) {
													...
													|| ( $r_sfrom < $r_rfrom && $r_sto > $r_rto )
														...
														$resource_disabled_time_slots[ $resource_id ][ $a_value ][] = $timeslot_for_date['from'] . ' - ' . $timeslot_for_date['to'];
														$rdisable_date_count++;
													}
												}
											}

											if ( 'duration_time' === $booking_type && 0 == $value['bookable'] ) {
												$resource_disabled_time_slots[ $resource_id ][ $a_value ][] = $value['from'] . ' - ' . $value['to'];
											}
			...
			$additional_data['resource_disable_time_slots'] = $resource_disabled_time_slots;
			...
			if ( isset( $booking_settings['enable_inline_calendar'] ) && $booking_settings['enable_inline_calendar'] == 'on' && '' === $booking_settings['bkap_date_in_dropdown'] ) {
				...
				if ( $session_start_date ) {
					...
					$hidden_date             = gmdate( 'j-n-Y', $session_start_strtotime );
				...
				if ( $session_end_date ) {
					...
					if ( isset( $booking_settings['booking_enable_multiple_day'] ) && $booking_settings['booking_enable_multiple_day'] == 'on' ) {
						...
						$hidden_date_checkout = gmdate( 'j-n-Y', $session_end_strtotime );
				...
				if ( $session_start_date ) {
					$hidden_date               = gmdate( 'j-n-Y', strtotime( $session_start_date ) );
					$first_available_date      = self::bkap_first_available( $product_id, $lockout_dates_array, $hidden_date );
						...
						if ( $start_ts == $end_ts ) {
							...
							if ( is_plugin_active( 'bkap-rental/rental.php' ) ) {
								...
								if ( isset( $booking_settings['booking_charge_per_day'] ) && $booking_settings['booking_charge_per_day'] == 'on' && isset( $booking_settings['booking_same_day'] ) && $booking_settings['booking_same_day'] == 'on' ) {
									$hidden_date_checkout = gmdate( 'j-n-Y', strtotime( $session_end_date ) );
								} else {
									$next_end_date        = strtotime( '+1 day', strtotime( $session_end_date ) );
									$hidden_date_checkout = gmdate( 'j-n-Y', $next_end_date );
								}
							} else {
								$next_end_date        = strtotime( '+1 day', strtotime( $session_end_date ) );
								$hidden_date_checkout = gmdate( 'j-n-Y', $next_end_date );
							...
							&& $booking_settings['booking_minimum_number_days_multiple'] > 0
								...
								if ( count( $number_of_days ) >= $booking_settings['booking_minimum_number_days_multiple'] ) {
									...
									$hidden_date_checkout   = gmdate( 'j-n-Y', $end_ts );
									$min_search_checkout    = $hidden_date_checkout;
								}
							} else {
								$hidden_date_checkout = gmdate( 'j-n-Y', strtotime( $session_end_date ) );
								...
								if ( isset( $booking[0]['hidden_date_checkout'] ) ) {
									...
									if ( strtotime( $hidden_date_checkout ) == strtotime( $hidden_date ) ) {
										if ( ! ( isset( $booking_settings['booking_charge_per_day'] ) && isset( $booking_settings['booking_same_day'] ) ) ) {
											$hidden_date_checkout = gmdate( 'j-n-Y', strtotime( $hidden_date_checkout . ' +1 day' ) );
										}
									}
								}
							}
			...
			if ( isset( $booking_settings['booking_enable_multiple_day'] ) && 'on' == $booking_settings['booking_enable_multiple_day'] ) {
				...
				$block_dates = (array) apply_filters( 'bkap_block_dates', $product_id, $booked_enddates );
			...
			wp_register_script( 'bkap-init-datepicker', BKAP_Files::rewrite_asset_url( '/assets/js/initialize-datepicker.js', BKAP_FILE ), '', BKAP_VERSION, false );
			...
			if ( $edit ) { // if a booking post is being edited
				...
				$booking_post_id = ( $is_called_from_vue && isset( $_REQUEST['booking_post_id'] ) && '' != $_REQUEST['booking_post_id'] ) ? $_REQUEST['booking_post_id'] : 0;
				...
				if ( $booking_post_id > 0 ) {
					...
					$start_date                        = gmdate( 'j-n-Y', strtotime( get_post_meta( $booking_post_id, '_bkap_start', true ) ) );
					...
					if ( isset( $booking_settings['booking_enable_multiple_day'] ) && $booking_settings['booking_enable_multiple_day'] == 'on' ) { // multiple days
						...
						$booking_end                           = gmdate( 'j-n-Y', strtotime( get_post_meta( $booking_post_id, '_bkap_end', true ) ) );
						...
						foreach ( $booking_range as $date ) {
							 $date_to_check = gmdate( 'j-n-Y', strtotime( $date ) );
							...
							if ( $resource_id != 0 && isset( $additional_data['bkap_booked_resource_data'] ) ) {
								if ( isset( $additional_data['bkap_booked_resource_data'][ $resource_id ]['bkap_locked_dates'] ) && is_string( $additional_data['bkap_booked_resource_data'][ $resource_id ]['bkap_locked_dates'] ) && strpos( $additional_data['bkap_booked_resource_data'][ $resource_id ]['bkap_locked_dates'], $date_to_check ) !== false ) {
					...
					if ( isset( $booking_settings['booking_enable_time'] ) && ( 'on' === $booking_settings['booking_enable_time'] || 'dates_time' === $booking_settings['booking_enable_time'] ) ) {
						...
						// get the time.
						if ( $time_format === '12' ) {
							$start_time = gmdate( 'h:i A', strtotime( get_post_meta( $booking_post_id, '_bkap_start', true ) ) );
							$end_time   = gmdate( 'h:i A', strtotime( get_post_meta( $booking_post_id, '_bkap_end', true ) ) );
						} else {
							$start_time = gmdate( 'H:i', strtotime( get_post_meta( $booking_post_id, '_bkap_start', true ) ) );
							$end_time   = gmdate( 'H:i', strtotime( get_post_meta( $booking_post_id, '_bkap_end', true ) ) );
						}
			...
			$variation_holidays_data = array();
			if ( $product_type === 'variable' ) {
				$variations = $_product->get_available_variations();

				foreach ( $variations as $var_key => $var_val ) {
					$variation_id       = $var_val['variation_id'];
					$variation_holidays = get_post_meta( $variation_id, '_holidays', true );
					$variation_id       = bkap_common::bkap_get_variation_id_based_on_selected_lang( $variation_id );

					if ( '' !== $variation_holidays ) {
						$variation_holidays_data[ $variation_id ] = $variation_holidays;
					}
				}
			}

			$booking_settings['variation_holidays_data'] = $variation_holidays_data;
			...
			if ( isset( $_GET['bkap_date'] ) ) {
				$hidden_dates_array['hidden_date']          = gmdate( 'j-n-Y', strtotime( $_GET['bkap_date'] ) );
				$hidden_dates_array['hidden_date_dropdown'] = gmdate( $date_format_set, strtotime( $_GET['bkap_date'] ) );
			} else {
				$hidden_dates_array['hidden_date_dropdown'] = $hidden_dates_array['hidden_date'];
			}

			$additional_data['bkap_no_of_days_count_from_current_day'] = apply_filters( 'bkap_no_of_days_count_from_current_day', false );
			...
			$additional_data['bkap_error_notice_time'] = apply_filters( 'bkap_time_for_error_notice_to_be_displayed', 5000, $product_id, $booking_settings );
			...
			$vue_data[ $init_datepicker_param ] = array(
			...
			wp_localize_script(
				...
				apply_filters(
					...
					$vue_data[ $init_datepicker_param ]
				)
			);
			...
			$vue_data[ $process_param_name ] = array(
			...
			$vue_data['additional_data']           = $additional_data;
			$vue_data['hidden_dates_array']        = $hidden_dates_array;

			return $is_called_from_vue ? $vue_data : $hidden_dates_array;
		}
		...
		public static function set_fixed_block_hidden_date( $hidden_date, $product_id, $holiday_array, $global_holidays, $lockout_dates_array ) {
			...
			if ( count( $results ) > 0 ) {
				...
				$min_day = gmdate( 'w', strtotime( $hidden_date ) );
				...
				if ( $fix_min_day != 'any_days' ) {
					...
					for ( $i = 0;; $i++ ) {
						...
						if ( in_array( $hidden_date, $holiday_array ) || in_array( $hidden_date, $global_holidays ) || in_array( $hidden_date, $lockout_dates_array ) ) {
							$hidden_date  = gmdate( 'j-n-Y', strtotime( '+1day', strtotime( $hidden_date ) ) );
							$date_updated = 'YES';
							...
							if ( $min_day == $fix_min_day ) {
								$hidden_date = gmdate( 'j-n-Y', strtotime( $hidden_date ) );
								break;
							} else {
								$hidden_date  = gmdate( 'j-n-Y', strtotime( '+1day', strtotime( $hidden_date ) ) );
								$date_updated = 'YES';
		...
		public static function bkap_date_datetime_price() {
			...
			$booking_date        = gmdate( 'Y-m-d', strtotime( $booking_date_format ) );
		...
		public static function bkap_date_lockout( $post = array() ) {
			...
			$bkap_type                = bkap_type( $product_id );
			$date_format_set          = bkap_common::bkap_get_date_format();
			$checkin_date             = sanitize_text_field( isset( $post['date'] ) ? $post['date'] : $_POST['date'] );
			$date                     = strtotime( $checkin_date );
			$date_check_in            = gmdate( 'Y-m-d', $date );
			$check_in_date            = gmdate( $date_format_set, $date );
			...
			$show_disabled_dates      = isset( $_POST['show_disabled_dates'] ) ? wp_unslash( sanitize_text_field( $_POST['show_disabled_dates'] ) ) : '';

			if ( 'after' === $resource_id ) {
				$message = in_array( $bkap_type, array( 'date_time', 'duration_time', 'multidates_fixedtime' ) ) ? __( 'Please select a slot.', 'woocommerce-booking' ) : __( 'Please select a resource.', 'woocommerce-booking' );
				$data = apply_filters(
			...
			$checkout_date  = isset( $post['end_date'] ) ? $post['end_date'] : ( isset( $_POST['end_date'] ) ? sanitize_text_field( $_POST['end_date'] ) : '' );
			$date_check_out = '';
			$check_out_date = '';
			if ( '' !== $checkout_date ) {
				$end_date       = strtotime( $checkout_date );
				$date_check_out = gmdate( 'Y-m-d', $end_date );
				$check_out_date = gmdate( $date_format_set, $end_date );
			}
			...
			if ( '' !== $checkin_date ) {
				...
				$available_tickets = 'on' === $show_disabled_dates && ! is_array( $available_tickets ) ? 'Unlimited' : $available_tickets;
				...
				if (
					...
					foreach ( $available_tickets as $_resource_id => $availability ) {
						$availability = 'on' === $show_disabled_dates ? 'U' : $availability;
						...
						// Translators: %1$s is the availability message, %2$s is the resource name.
						$message .= '<label>' . sprintf( __( '%1$s for %2$s', 'woocommerce-booking' ), $_message, $resource_name ) . '</label>';
					}
					...
					if ( is_numeric( $available_tickets ) || 'Unlimited' === $available_tickets ) {
						...
						if ( '' !== $check_out_date) {
							$message .= sprintf( __( ' to %s', 'woocommerce-booking' ), $check_out_date );
						}
					}

					if ( strlen( $available_tickets ) > 15 ) {
			...
			// Fix#6366
			$booking_txt = $available_tickets > 1 ? 'bookings are' : 'booking is';
			$message     = str_replace( 'booking(s) are', $booking_txt, $message );
		...
		public static function bkap_get_date_availability(
			...
			$selected_date            = gmdate( 'j-n-Y', strtotime( $hidden_date ) );
			...
			if ( 'YES' === $check_availability ) {
				...
				if ( $resource_id ) {
					...
					if ( ! is_array( $resource_id ) ) {
						...
						if ( $cal_price ) { //phpcs:ignore
							...
							$qty_in_cart            = Class_Bkap_Product_Resource::return_quantity_in_cart( $resource_id, $hidden_date, '', $product_id );
					...
					foreach ( $resource_id as $id ) {
						...
						if ( '' != $hidden_end_date ) {
							$hidden_end_date = gmdate( 'j-n-Y', strtotime( $hidden_end_date ) );
						}

						$qty_in_placed_bookings           = is_array( $resource_bookings_placed ) && isset( $resource_bookings_placed[ $id ] ) ? Class_Bkap_Product_Resource::return_quantity_in_placed_bookings( $resource_bookings_placed[ $id ], $selected_date, false, $hidden_end_date ) : 0;
						$qty_in_cart                      = Class_Bkap_Product_Resource::return_quantity_in_cart( $id, $hidden_date, $hidden_end_date, $product_id );
						$final_resource_availability      = $bkap_resource_availability - $qty_in_placed_bookings - $qty_in_cart;
						$all_resource_availability[ $id ] = $final_resource_availability > 0 ? $final_resource_availability : 0;
					}
				...
				if ( isset( $booking_settings['booking_enable_multiple_day'] ) && 'on' === $booking_settings['booking_enable_multiple_day'] ) {
					...
					if ( '' != $hidden_end_date ) {
						$selected_date_range = bkap_common::bkap_get_betweendays( $hidden_date, $hidden_end_date, 'j-n-Y' );
						foreach ( $selected_date_range as $key => $value ) {
							$specific_available_tickets = bkap_get_specific_date_maximum_booking( $available_tickets, $value, $product_id, $booking_settings );
							if ( $available_tickets > $specific_available_tickets ) {
								$available_tickets = $specific_available_tickets;
							}
						}
					} else {
					...
					if ( $cart_check ) { // if cart_check = true it means this has been called from front end product page.
						...
						$dates_new_array = bkap_get_booked( $product_id, true, true );
						$dates_new_array = $dates_new_array['dates'];

						$booked_qty = 0;
						if ( '' != $hidden_end_date ) {
							$hidden_end_date = gmdate( 'Y-m-d', strtotime( $hidden_end_date ) );
							$selected_date_range = bkap_common::bkap_get_betweendays( $hidden_date, $hidden_end_date, 'Ymd' );

							$booked_ymd_qty = array();
							foreach ( $selected_date_range as $key => $value) {
								if ( isset( $dates_new_array[$value] ) ) {
									$booked_ymd_qty[ $value ] = $dates_new_array[ $value ];
								}
							}
							if ( count( $booked_ymd_qty ) > 0 ) {
								$booked_qty = max( $booked_ymd_qty );
							}
						} elseif ( isset( $dates_new_array[ gmdate( 'Ymd', strtotime( $hidden_date ) ) ] ) ) {
							$booked_qty = $dates_new_array[ gmdate( 'Ymd', strtotime( $hidden_date ) ) ];
						}

						// If records are found then the availability needs to be subtracted from the total lockout value.
						if ( $available_tickets > 0 ) {
							$unlimited         = false;
							$available_tickets = $available_tickets - $booked_qty;
						}
					...
					if ( ! $booking_available ) {
						...
						if ( count( $bookings_data ) > 0 ) {
							if ( isset( $bookings_data[ gmdate( 'Ymd', strtotime( $hidden_date ) ) ] ) ) {
								$bookings_done = $bookings_data[ gmdate( 'Ymd', strtotime( $hidden_date ) ) ];
					...
					if ( isset( $results_date ) && count( $results_date ) > 0 ) {
						...
						$manage_time_availability_present = false;
						if ( isset( $booking_settings['bkap_manage_time_availability'] ) && ! empty( $booking_settings['bkap_manage_time_availability'] ) ) {
							$mta_availability_data = apply_filters( 'bkap_manage_time_availability_settings', $booking_settings['bkap_manage_time_availability'], $booking_settings, $product_id );
							if ( is_array( $mta_availability_data ) && count( $mta_availability_data ) > 0 ) {
								$manage_time_availability_present = true;
							}
						}

						$date_time_considered = array();
						...
						foreach ( $results_date as $key => $value ) {
							...
							if ( $manage_time_availability_present ) {
								$value_to_time = ( '' !== $value->to_time ) ? $value->to_time : '23:59';
								$timeslots_str = $value->from_time . ' - ' . $value_to_time . '|';
								$timeslots_str = bkap_filter_time_based_on_resource_availability( $hidden_date, $mta_availability_data, $timeslots_str, array( 'type' => 'fixed_time' ), 0, $product_id, $booking_settings );
								if ( '' == $timeslots_str ) {
									continue;
								}
							}

							if ( isset( $value->from_time ) && isset( $date_time_considered[ $value->from_time . ' - ' . $value->to_time ] ) ) { // incase duplicate records in db.
								continue;
							}
							...
							if ( isset( $value->from_time ) ) {
								$date_time_considered[ $value->from_time . ' - ' . $value->to_time ] = $value->total_booking;
							}
						...
						$weekday         = gmdate( 'w', strtotime( $hidden_date ) );
						$booking_weekday = 'booking_weekday_' . $weekday;
		...
		public static function bkap_insert_date_record( $date, $product_id, $booking_settings, $product ) {
			...
			if ( $product_type === 'bundle' ) {
				$cart_configs = bkap_common::bkap_bundle_add_to_cart_config( $product, $product_id );
		...
		public static function bkap_get_time_lockout( $post = array() ) {
			...
			if ( 'after' === $resource_id ) {
				$message = __( 'Please select the resource.' );
				$data = apply_filters(
					'bkap_date_lockout_data',
					array(
						'error' => true,
			...
			if ( '' !== $timeslots ) {
				...
				$booking_date        = gmdate( 'Y-m-d', $date ); // Checkin/Booking Date
				$booking_date_disply = gmdate( $date_format_set, $date );
				...
				if ( isset( $variation_id ) && $variation_id > 0 ) {
					...
					if ( isset( $variation_lockout ) && $variation_lockout > 0 ) {
						...
						$available_tickets = $variation_lockout;
						$date_check_in     = gmdate( 'j-n-Y', $date );
						...
						foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
							...
							if ( isset( $values['bkap_booking'] ) ) {
								...
								if ( isset( $cart_booking['resource_id'] ) && in_array( $cart_booking['resource_id'], $resource_ids ) ) {
									...
									$time_slot = isset( $cart_booking['time_slot'] ) ? $cart_booking['time_slot'] : '00:01 - 23:59';
									$timeslot  = explode( ' - ', $time_slot );
									$frmtime   = gmdate( 'H:i', strtotime( $timeslot[0] ) );
									$totime    = isset( $timeslot[1] ) ? gmdate( 'H:i', strtotime( $timeslot[1] ) ) : '00:00';
									...
									$resource_date_array[ $cart_booking['resource_id'] ][ $cart_booking['hidden_date'] ][ $time_slot ] = $_qty + $values['quantity'];
								}
							}
						}
					}
				...
				for ( $i = 0; $i < $time_slot_array_count; $i++ ) {
					...
					if ( 'YES' == $check_availability ) {
						...
						if ( $booking_id > 0 ) {
							...
							$bkap_start = gmdate( 'H:i', $bkap->start );
							$bkap_end   = gmdate( 'H:i', $bkap->end );
							/*
							 if ( '00:00' == $bkap_end ) {
								$bkap_end = '';
							} */
									...
									if ( $booking['hidden_date'] == $booking_date_in ) {
										if ( $booking['time_slot'] == $timeslot ) {
											...
											if ( 'Unlimited' == $available_tickets ) {
												$unlimited = 'YES';
											} elseif ( (int) $available_tickets > 0 ) {
												$unlimited          = 'NO';
												$available_tickets -= $quantity;
					...
					if ( '' !== $resource_id ) {
						...
						if ( $booking_in_cart == 'YES' && $available_tickets == 0 ) {
							$available_tickets = 0;
							$message           = sprintf( __( 'Available bookings for this timeslot are already added in the cart. You can add bookings for a different timeslot or place the order that is currently in the <a href="%1$s">%2$s</a>.', 'woocommerce-booking' ), esc_url( wc_get_page_permalink( 'cart' ) ), esc_html__( 'cart', 'woocommerce-booking' ) );
			...
			if ( 'on' === $show_disabled_dates ) {
				$message           = str_replace(  $available_tickets .' booking(s)', 'Unlimited booking(s)', $message );
				$available_tickets = __( 'Unlimited', 'woocommerce-booking' );
			}
		...
		public static function bkap_get_time_availability( $product_id, $booking_date, $from_hrs, $to_hrs, $check_availability ) {
			...
			if ( 'YES' == $check_availability ) {
				...
				$from_hrs = gmdate( 'H:i', strtotime( $from_hrs ) );
				...
				if ( $to_hrs == '' ) {
					...
					$to_hrs       = gmdate( 'H:i', strtotime( $to_hrs ) );
					$time_query   = 'SELECT total_booking,available_booking FROM `' . $wpdb->prefix . "booking_history`
								WHERE post_id = %d
								AND weekday = ''
								AND start_date = %s
				...
				if ( isset( $results_time ) && count( $results_time ) > 0 ) {
					...
					if ( $results_time[0]->available_booking > 0 ) {
						...
						if ( count( $bookings_data ) > 0 ) {
							if ( isset( $bookings_data[ gmdate( 'Ymd', strtotime( $booking_date ) ) ] ) ) {
								if ( isset( $bookings_data[ gmdate( 'Ymd', strtotime( $booking_date ) ) ][ $from_hrs . ' - ' . $to_hrs ] ) ) {
									$available = $bookings_data[ gmdate( 'Ymd', strtotime( $booking_date ) ) ][ $from_hrs . ' - ' . $to_hrs ];
					...
					$weekday         = gmdate( 'w', strtotime( $booking_date ) );
					$booking_weekday = 'booking_weekday_' . $weekday;
		...
		public static function bkap_get_per_night_price( $post_data = array() ) {
			...
			$checkin_date      = gmdate( 'Y-m-d', $checkin_date_str );
			$checkout_date     = gmdate( 'Y-m-d', $checkout_date_str );
			...
			$number = apply_filters( 'bkap_selected_number_of_nights', $number, $checkin_date, $checkout_date, $product_id, $booking_settings );
		...
		public static function bkap_before_get_per_night_price_callback() {
			...
			if ( ( 0 === (int) $variation_id_to_fetch || '' === $variation_id_to_fetch ) && 'variable' === $product_type ) {
		...
		public static function bkap_insert_date( $p_id = 0, $p_type = '', $date = '' ) {
			...
			$date_to_check = gmdate( 'Y-m-d', strtotime( $current_date ) );
			...
			if ( ! $results_check ) {
				...
				$day_check         = 'booking_weekday_' . gmdate( 'w', strtotime( $current_date ) );
		...
		public static function bkap_check_for_time_slot( $data = array() ) {
			...
			if ( isset( $_POST['date_time_type'] ) && $_POST['date_time_type'] == 'duration_time' ) {
				$duration_time_array = Bkap_Duration_Time::get_duration_time_slot( $current_date, $post_id, $data );
			} else {
		...
		public static function bkap_display_time_dropdown_timezone( $current_date, $date_time_drop_down, $global_settings, $extra_information ) {
			...
			if ( count( $date_time_drop_down ) > 0 ) { // Dates mate timeslots chhe ke nai
				...
				$current_date_ymd = gmdate( 'Y-m-d', $current_date_str ); // getting date in Y-m-d format
					...
					if ( trim( $time_drop_down_array[0] ) === 'ERROR' ) {
						...
						foreach ( $time_drop_down_array as $k => $v ) {
							...
							if ( $v != '' ) {
								...
								$datetimeISO = gmdate( DateTime::ISO8601, $fromtime );
								$timedateymd = gmdate( 'Y-m-d', $offset + $fromtime );
								
								if ( $timedateymd != $current_date_ymd ) {
									continue;
								}

								$from_time = bkap_convert_date_from_timezone_to_timezone( $key . ' ' . $vexplode[0] , $extra_information['store_time_zone'], Bkap_Timezone_Conversion::get_timezone_var( 'bkap_timezone_name' ), $time_format_to_show );
								$to_time   = '';
								if ( isset( $vexplode[1] ) ) {
									$to_time = ' - ' . bkap_convert_date_from_timezone_to_timezone( $key . ' ' . $vexplode[1] , $extra_information['store_time_zone'], Bkap_Timezone_Conversion::get_timezone_var( 'bkap_timezone_name' ), $time_format_to_show );
								}
		...
		public static function get_time_slot( $current_date, $post_id, $return_unavailable_time_slots = false ) {
			...
			$time_format_to_show  = bkap_common::bkap_get_time_format( $global_settings );
			$current_time         = current_time( 'timestamp' );
			$jnyDate              = gmdate( 'j-n-Y', $current_time );
			...
			$dropdownarray        = array();
			$time_slot            = '';
			$show_disabled_dates  = isset( $_POST['show_disabled_dates'] ) ? $_POST['show_disabled_dates'] : '';
			...
			if ( $return_unavailable_time_slots ) {
				...
				if ( count( $results_check ) > 0 ) {
					foreach ( $results_check as $key => $value ) {
						...
						$from_time = '' !== $value->from_time ? gmdate( $time_format_to_show, strtotime( $value->from_time ) ) : '';
						$to_time   = '' !== $value->to_time ? gmdate( $time_format_to_show, strtotime( $value->to_time ) ) : '';
			...
			$available_booking = 'AND available_booking > 0 ';
			...
			$check_query   = 'SELECT * FROM `' . $wpdb->prefix . "booking_history`
            				...
            				AND post_id = '" . $post_id . "'
            				AND status = ''
            				" . $available_booking . "ORDER BY STR_TO_DATE(from_time,'%H:%i')";
			$results_check = $wpdb->get_results( $check_query );
			...
			if ( count( $results_check ) > 0 ) {
				...
				foreach ( $results_check as $key => $value ) {
					if ( $value->weekday == '' ) {
						...
						if ( $value->from_time != '' ) {
							$from_time_show     = gmdate( $time_format_to_show, strtotime( $value->from_time ) );
							$from_time_db_value = gmdate( $time_format_db_value, strtotime( $value->from_time ) );
						}
						...
						if ( $include ) {
							...
							if ( $to_time_show != '' ) {
								$to_time_show     = gmdate( $time_format_to_show, strtotime( $value->to_time ) );
								$to_time_db_value = gmdate( $time_format_db_value, strtotime( $value->to_time ) );
				...
				if ( ! $specific ) { // if no records found based on specific date then it will go in this if..
					...
					foreach ( $results_day_check as $k => $v ) {
						...
						$from_hi = gmdate( 'H:i', strtotime( $v->from_time ) );
						$to_hi   = ( $v->to_time != '' ) ? gmdate( 'H:i', strtotime( $v->to_time ) ) : '';
					...
					foreach ( $results_day_check as $k => $v ) {
						...
						foreach ( $results_check as $key => $value ) {
							...
							if ( $v->from_time != '' && $v->to_time != '' ) {
								$from_time_chk = gmdate( $time_format_db_value, strtotime( $v->from_time ) );
								if ( $value->from_time == $from_time_chk ) {

									if ( $v->to_time != '' ) {
										$to_time_chk = gmdate( $time_format_db_value, strtotime( $v->to_time ) );
									}
				...
				if ( $results_day_check ) {
					...
					if ( 'on' !== $show_disabled_dates && count( $results_check ) == count( $results_day_check ) && ! ( isset( $_POST['resource_id'] ) && '' !== $_POST['resource_id'] ) ) {
				...
				if ( isset( $_POST['variation_timeslot_lockout'] ) && $_POST['variation_timeslot_lockout'] != '' ) {
					...
					foreach ( $dates_array as $date_key => $date_value ) {
						...
						if ( stripslashes( $list_dates[0] ) == $_POST['current_date'] ) {
							...
							$time_slot         = gmdate( $time_format_to_show, strtotime( $time_slot_explode[0] ) );
							$time_slot        .= ' - ' . gmdate( $time_format_to_show, strtotime( $time_slot_explode[1] ) );
			...
			if ( isset( $booking_settings['bkap_manage_time_availability'] ) && ! empty( $booking_settings['bkap_manage_time_availability'] ) ) {
				$mta_availability_data = apply_filters( 'bkap_manage_time_availability_settings', $booking_settings['bkap_manage_time_availability'], $booking_settings, $post_id );
				if ( is_array( $mta_availability_data ) && count( $mta_availability_data ) > 0 ) {
			...
			if ( '' !== $resource_id ) {
				...
				if ( 'after' == $resource_id ) {
					$bkap_booked_resource_data = isset( $_POST['bkap_booked_resource_data'] ) ? wp_unslash( $_POST['bkap_booked_resource_data'] ) : '';
					$bkap_booked_resource_data = json_decode( $bkap_booked_resource_data, true );
					if ( isset( $bkap_booked_resource_data[ $current_date ] ) ) {
						$resource_disabled_time_slots_data = $bkap_booked_resource_data[ $current_date ];
						foreach ( $resource_disabled_time_slots_data as $key => $value ) {
							$time_exploded      = explode( ' - ', $value );
							$from_time_exploded = bkap_common::bkap_get_formated_time( $time_exploded[0] );
							$to_time_exploded   = isset( $time_exploded[1] ) ? bkap_common::bkap_get_formated_time( $time_exploded[1] ) : '';
							$formatted_value    = '' !== $to_time_exploded ? $from_time_exploded . ' - ' . $to_time_exploded : $from_time_exploded;
							if ( isset( $dropdownarray[ $formatted_value ] ) ) {
								unset( $dropdownarray[ $formatted_value ] );
								if ( strpos( $timeslots_str, $formatted_value ) >= 0 ) {
									$pattern_to_be_removed = $formatted_value . '|';
									$timeslots_str         = str_replace( $pattern_to_be_removed, '', $timeslots_str );
								}
							}
						}
					...
					foreach ( $resource_ids as $id ) {
						...
						$time_slots[ $id ] = array_filter(
							array_map( 'sanitize_text_field', explode( '|', $_timeslots_str ) ),
							function( $value ) {
								return $value !== '';
							}
						);
						...
						if ( isset( $resource_lockoutdates[ $id ] ) ) {
							$locked_dates[ $id ] = array_filter(
								explode( ',', $resource_lockoutdates[ $id ] ),
							...
							foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
								...
								if ( 0 === $r_availability ) {
									...
									if ( $trc_qty >= $r_availability ) {
										...
										$totime                = isset( $timeslotexplode[1] ) ? gmdate( 'H:i', strtotime( $timeslotexplode[1] ) ) : '00:00'; // phpcs:ignore
										$_time_slot            = gmdate( 'H:i', strtotime( $timeslotexplode[0] ) ) . ' - ' . $totime; // phpcs:ignore
										$locked_dates[ $id ][] = '\"' . $current_date . '\"=>' . $_time_slot;
										$time_slots[ $id ][]   = $_time_slot;
									}
	
									$locked_dates[ $id ] = isset( $locked_dates[ $id ] ) 
										? array_filter(
											array_unique( $locked_dates[ $id ] ),
											static function( $value ) {
												return trim( $value ) !== '';
											}
										) 
										: array();
										$time_slots[ $id ] = isset( $time_slots[ $id ] ) 
											? array_filter(
												array_unique( $time_slots[ $id ] ),
												static function( $value ) {
													return trim( $value ) !== '';
												}
											) 
					...
					if ( count( $locked_dates ) > 0 && count( $time_slots ) > 0 ) {
						...
						foreach ( $resource_ids as $id ) {
							...
							if ( isset( $locked_dates[ $id ] ) && is_array( $locked_dates[ $id ] ) && count( $locked_dates[ $id ] ) > 0 ) {
								...
								foreach ( $locked_dates[ $id ] as $dates ) {
									...
									if ( $_date[0] === $_POST['current_date'] ) { // phpcs:ignore
										...
										$_time_slot  = gmdate( $time_format_to_show, $lf ); // phpcs:ignore
										$_time_slot .= ' - ' . gmdate( $time_format_to_show, $lt ); // phpcs:ignore
										...
										*/
										...
										foreach ( $time_slots[ $id ] as $key => $_timeslot ) {
											...
											$overlapping_remove_item = ( ( $f > $lf && $f < $lt ) || ( $t > $lf && $t < $lt ) || ( $f <= $lf && $t >= $lt ) ) ? true : $remove_item;
											$overlapping_remove_item = apply_filters( 'bkap_remove_overlapping_timeslot_for_resource', $overlapping_remove_item, array( 'f' => $f, 't' => $t, 'lf' => $lf, 'lt' => $lt, 'remove_item' => $remove_item ), $id );
											if ( $remove_item || $overlapping_remove_item ) {
												unset( $time_slots[ $id ][ $key ] );
											}
										}
									}
						...
						if ( 0 === count( $uniform_time_slots ) ) {
							...
							if ( wp_doing_ajax() ) {
								$wp_send_json['bkap_time_count']    = 0;
								$wp_send_json['bkap_time_dropdown'] = $_error_message;
								wp_send_json( $wp_send_json );
							}
						}
			...
			if ( $check_for_overlapping_timeslots ) {
				...
				if ( '' !== $timeslots_str && $overlapping && '' === $resource_id ) { // #6171.

					$times_array = explode( '|', $timeslots_str );
					array_pop( $times_array );
	
					$timeslots = $times_array;
					...
					$time_records    = $wpdb->get_results( $wpdb->prepare( $query_unlimited, $post_id, $date_ymd, '%H:%i' ) ); // phpcs:ignore
	
					$time_slot_settings = $booking_settings['booking_time_settings'];
					$weekday            = bkap_weekday_string( $date_ymd );
	
					$all_timeslots_data_weekdays = array();
					if ( isset( $time_slot_settings[ $weekday ] ) ) {
						$all_timeslots_data_weekdays = $time_slot_settings[ $weekday ];
					} elseif ( isset( $time_slot_settings[ gmdate( 'j-n-Y', strtotime( $date_ymd ) ) ] ) ) {
						$all_timeslots_data_weekdays = $time_slot_settings[ gmdate( 'j-n-Y', strtotime( $date_ymd ) ) ];
					}
					...
					foreach ( $timeslots as $key => $value ) {
						...
						$from_time_db = gmdate( 'H:i', strtotime( $date_ymd . ' ' . $from_time ) );
						$to_time_db   = '' !== $to_time ? gmdate( 'H:i', strtotime( $date_ymd . ' ' . $to_time ) ) : '00:00';
						$timeslothi   = $from_time_db . ' - ' . $to_time_db;
	
						foreach ( $all_timeslots_data_weekdays as $tk => $tv ) {
							$settings_timeslot = $tv['from_slot_hrs'] . ':' . $tv['from_slot_min'] . ' - ' . $tv['to_slot_hrs'] . ':' . $tv['to_slot_min'];
	
							if ( $settings_timeslot === $timeslothi ) {
								$lockout = $tv['lockout_slot'];
								if ( isset( $dropdownarray[ $value ] ) && 'unlimited' !== $dropdownarray[ $value ] ) {
									$lockout = $dropdownarray[ $value ];
								}
							}
						}
						$booked_slots = 0;
						foreach ( $time_records as $records ) {
							$dbfromtime = $records->from_time;
							$dbtotime   = '' !== $records->to_time ? $records->to_time : '00:00';
							$dbtimeslot = $dbfromtime . ' - ' . $dbtotime;
	
							if ( $dbtimeslot === $timeslothi ) {
								continue;
							}
	
							if ( strtotime( $records->from_time ) >= strtotime( $from_time_db ) && strtotime( $records->from_time ) < strtotime( $to_time_db ) ) {
								$booking      = $records->total_booking - $records->available_booking;
								$booked_slots = $booked_slots + $booking;
							}
						}
	
						if ( isset( $lockout ) && '' !== $lockout && $lockout <= $booked_slots ) {
							unset( $times_array[ $key ] );
						}
					}
	
					if ( count( $times_array ) > 0 ) {
						$timeslots_str  = implode( '|', $times_array );
						$timeslots_str .= '|';
					}
				}
			}

			/* To make sure the unwanted timeslots doesn't show on front end for booking */
			if ( ! isset( $dropdownarray['ERROR'] ) ) {
			
				$alltimeslots = bkap_get_timeslots_weekdays( $booking_settings );

				if ( isset( $alltimeslots[ $current_date ] ) ) {
					$booking_time_slots = $alltimeslots[ $current_date ];
				} else {
					$weekday = bkap_weekday_string( $current_date );
					if ( isset( $alltimeslots[ $weekday ] ) ) {
						$booking_time_slots = $alltimeslots[ $weekday ];
					}
				}

				if ( isset( $booking_time_slots ) && count( $booking_time_slots ) > 0 ) {

					$available_dropdown  = $dropdownarray;
					$time_format_to_show = bkap_common::bkap_get_time_format( $global_settings );
					$changed             = false;
					foreach ( $available_dropdown as $akey => $avalue ) {

						if ( '12' === $global_settings->booking_time_format ) {
							$explode_time = explode( ' - ', $akey );
							if ( isset( $explode_time[1] ) ) {
								$from_time    = gmdate( 'H:i', strtotime( $current_date . ' ' . $explode_time[0] ) );
								$to_time      = gmdate( 'H:i', strtotime( $current_date . ' ' . $explode_time[1] ) );
								$booking_time = $from_time . ' - ' . $to_time;
							} else {
								$booking_time = gmdate( 'H:i', strtotime( $current_date . ' ' . $explode_time[0] ) );
						...
						if ( ! in_array( $booking_time, $booking_time_slots ) ) {
							$changed = true;
							unset( $dropdownarray[ $akey ] );
						}
					}
					if ( $changed ) {
						$timeslots_str = ( count( $dropdownarray ) > 0 ) ? implode( '|', array_keys( $dropdownarray ) ) . '|' : '';
		...
		public static function bkap_prepopulate_fixed_block( $duplicate_of ) {
			...
			if ( $session_start_date ) {
				...
				$day_chosen = gmdate( 'N', strtotime( $session_start_date ) );
		...
		public static function bkap_first_available( $duplicate_of, $lockout_dates_array, $date ) {
			...
			if ( isset( $booking_settings['bkap_all_data_unavailable'] ) && 'on' === $booking_settings['bkap_all_data_unavailable'] ) {
				if ( isset( $booking_settings['bkap_manage_time_availability'] ) ) {
					...
					$mta_availability_data = apply_filters( 'bkap_manage_time_availability_settings', $booking_settings['bkap_manage_time_availability'], $booking_settings, $duplicate_of );

					if ( empty( $mta_availability_data ) ) {
						return $date;
					} else {
						// if range is added and all added ranges are non-bookable then return $date.
						$bookable_range_available = false;
						foreach ( $mta_availability_data as $key => $value ) {
							if ( $value['bookable'] == 1 ) {
			...
			if ( $booking_dates_arr != '' && count( $booking_dates_arr ) > 0 && ( count( $holiday_array ) > 0 || count( $global_holidays ) > 0 ) ) {
				$booking_dates_arr = self::bkap_check_specificdate_in_holiday( $booking_dates_arr, $holiday_array, $global_holidays );
			}

			$date_updated              = '';
			$min_day                   = gmdate( 'w', strtotime( $date ) );
			$numbers_of_days_to_choose = isset( $booking_settings['booking_maximum_number_days'] ) ? $booking_settings['booking_maximum_number_days'] - 1 : '';
			$current_time              = strtotime( 'today midnight' );
			$current_time_ymd          = gmdate( 'Y-m-d', $current_time );

			/* Issue #5674 */
			$only_specific_date = false;
			if ( ! in_array( $booking_type, array( 'multiple_days', 'only_day', 'multidates' ) ) ) {
				if ( 'date_time' === $booking_type && count( $booking_dates_arr ) > 0 ) {
					$booking_time_settings = isset( $booking_settings['booking_time_settings'] ) ? $booking_settings['booking_time_settings'] : array();
					$only_specific_date    = true;
					foreach ( $recurring_date as $key => $value ) {
						if ( 'on' === $value && isset( $booking_time_settings[$key] ) ) {
							$only_specific_date = false;
						}
					}
				}

				if ( $only_specific_date ) {
					$min_date         = bkap_closest_specific_date( $booking_dates_arr, strtotime( $date ) );
					$current_time     = strtotime( $min_date );
					$date             = gmdate( 'j-n-Y', $current_time );
					$current_time_ymd = gmdate( 'Y-m-d', $current_time );
				}
			}
			...
			for ( $i = 0; $i <= $diff_days; $i++ ) {
				...
				if ( $booking_type === 'date_time' && ! $custom_holiday_present && ! isset( $holiday_array[ $date ] ) && ! in_array( $date, $global_holidays ) ) {
				...
				if ( ( isset( $booking_settings['booking_recurring_booking'] ) && 'on' == $booking_settings['booking_recurring_booking'] && $booking_type != 'multiple_days' ) || apply_filters( 'bkap_consider_recurring_weekday_as_enabled', false, $duplicate_of, $booking_settings ) ) {
					...
					if ( ( isset( $recurring_date[ 'booking_weekday_' . $min_day ] ) && $recurring_date[ 'booking_weekday_' . $min_day ] == 'on' ) || apply_filters( 'bkap_consider_recurring_weekday_as_enabled', false, $duplicate_of, $booking_settings ) ) {
						$date_updated = ( $time_slots_missing  || in_array( $date, $global_holidays ) ) ? 'YES' : 'NO';
					} elseif ( is_array( $booking_dates_arr ) && count( $booking_dates_arr ) > 0 && $booking_type != 'multiple_days' ) {
						$date_updated = ( ! array_key_exists( $date, $booking_dates_arr ) || $time_slots_missing ) ? 'YES' : 'NO';
					}
					...
					if ( $booking_type == 'multiple_days' && $bkap_fixed_blocks == 'booking_fixed_block_enable' && count( $bkap_fixed_blocks_data ) > 0 ) {
						...
						if ( count( $bkap_fixed_blocks_data ) > 0 ) {
							...
							if ( $first_block['start_day'] == 'any_days' || $min_day == $first_block['start_day'] ) {
								...
								$fixed_min_day = gmdate( 'w', strtotime( $date ) );

								while ( $fix_min_day != $fixed_min_day ) {
									$date                  = gmdate( 'j-n-Y', strtotime( '+1day', strtotime( $date ) ) );
									$fixed_min_day         = gmdate( 'w', strtotime( $date ) );
									$date_updated          = 'NO';
						...
						if ( is_array( $booking_dates_arr ) && count( $booking_dates_arr ) > 0 && '' == $date_updated ) {
		...
		public static function bkap_add_unlimited_slots( $dropdown, $post_id, $booking_settings, $global_settings, $extra_information ) {
			...
			if ( $product_id > 0 ) {
				...
				$current_time = current_time( 'timestamp' );
				$date_today   = gmdate( 'Y-m-d H:i', $current_time );
		...
		function add_time_slot( $dropdown ) {
			...
			if ( $booking_id > 0 && get_post_type( $booking_id ) === 'bkap_booking' && isset( $_REQUEST['current_date'] ) ) {
				$display = self::bkap_add_time_slot_on_bookingpage_or_vieworder( $display, $_REQUEST['post_id'], $booking_id, $_REQUEST['current_date'] );
			} elseif ( isset( $_POST['bkap_page'] ) && 'view-order' == $_POST['bkap_page'] && isset( $_POST['post_id'] ) ) {
		...
		public static function bkap_add_time_slot_on_bookingpage_or_vieworder( $display, $product_id, $booking_id, $current_date ) {
			...
			$time_display    = gmdate( $time_format, strtotime( trim( $times_selected[0] ) ) );
			...
			if ( ! in_array( $time_display, $time_drop_down_array ) ) {
				...
				if ( $current_date !== gmdate( 'j-n-Y', strtotime( $booking->get_start() ) ) ) {

					$found         = false;
					$booking_date  = gmdate( 'j-n-Y', strtotime( $current_date ) );
					...
					if ( is_array( $slots_list ) && count( $slots_list ) > 0 ) {
						...
						foreach ( $slots_list as $times ) {
							...
							$from_time_check = gmdate( $time_format, strtotime( $times['from_slot_hrs'] . ':' . $times['from_slot_min'] ) );
							$to_time_check   = gmdate( $time_format, strtotime( $times['to_slot_hrs'] . ':' . $times['to_slot_min'] ) );
