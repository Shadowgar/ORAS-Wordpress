...
class BKAP_Booking {
	...
	public function __construct( $booking_data = false ) {
		...
		} elseif ( intval( $booking_data ) && 0 < $booking_data ) {
			...
			if ( count( $this->custom_fields ) == 0 ) {
				...
				foreach ( $results as $row ) {
					...
					if ( isset( $meta_data[ $meta_key ] ) ) {
						if ( ! is_array( $meta_data[ $meta_key ] ) ) {
							$meta_data[ $meta_key ] = array( $meta_data[ $meta_key ] );
						}
	...
	private function new_booking( $status, $order_data ) {
		...
		$booking_data = array(
			...
			'post_title'  => sprintf(
				...
				__( 'Booking &ndash; %s', 'woocommerce-booking' ),
				/* translators: %s: Date Format */
				$now->format( _x( '%1$b %2$d, %Y @ %I:%M %p', 'Booking date parsed by strftime', 'woocommerce-booking' ) )
			),
			'post_status' => $status,
			'ping_status' => 'closed',
	...
	public function get_customer() {
		$name    = __( 'Guest', 'woocommerce-booking' );
		$email   = '';
		$user_id = 0;
		...
		if ( $order = $this->get_order() ) {
			...
			$name = 0 !== absint( $user_id ) ? $name : ( 'Guest' === $name ? $name : sprintf( _x( '%s (Guest)', 'Guest string with name from booking order in brackets', 'woocommerce-booking' ), $name ) );
	...
	function get_time() {
		...
		$time_format = 'H:i';
		$start_time  = gmdate( $time_format, strtotime( $this->get_start() ) );
		$end_time    = gmdate( $time_format, strtotime( $this->get_end() ) );
	...
	function get_start_date( $global_settings = array() ) {
		...
		return gmdate( $date_format_set, strtotime( $start ) );
	...
	function get_end_date( $global_settings = array() ) {
		...
		if ( $start !== $end ) {
			...
			$global_settings = empty( $global_settings ) ? bkap_global_setting() : $global_settings;
			$date_format_set = $date_formats[ $global_settings->booking_date_format ];
			$end_date        = gmdate( $date_format_set, strtotime( $end ) );
		}
	...
	function get_persons_info() {
		...
		if ( ! empty( $person ) ) {
			if ( isset( $person[0] ) ) {
				$person_info = BKAP_Person::bkap_get_person_label() . ' : ' . $person[0];
			} else {
	...
	/**
	 * Returns Additional Comment information.
	 *
	 * @return string $additional_comment - Additional Comment Data.
	 * @since 6.4.0
	 */
	function get_additional_comment() {
		$additional_comment = get_post_meta( $this->id, '_bkap_additional_comment', true );
	...
	function get_selected_duration_time() {
		...
		$duration_time = gmdate( 'H:i', strtotime( $bkap_start ) );
