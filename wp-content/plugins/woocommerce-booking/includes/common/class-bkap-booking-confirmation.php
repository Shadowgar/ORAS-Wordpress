...
if ( ! class_exists( 'BKAP_Booking_Confirmation' ) ) {
	...
	class BKAP_Booking_Confirmation {
		...
		public function __construct() {
			...
			add_action( 'plugins_loaded', array( &$this, 'bkap_load_requires_confirmation_payment_gateway' ) );
		}

		/**
		 * Loads requires confirmation payment gateway class
		 *
		 * @since 6.0.0
		 */
		public function bkap_load_requires_confirmation_payment_gateway() {
			require BKAP_PLUGIN_PATH . '/includes/common/class-bkap-payment-gateway.php';
		}
			...
			<div id="requires_confirmation_section" class="booking_options-flex-main">
				...
				<div class="booking_options-flex-child">
					<label class="bkap_switch">
						<input type="checkbox" name="bkap_requires_confirmation" id="bkap_requires_confirmation" <?php echo esc_attr( $requires_confirmation ); ?>>
						<div class="bkap_slider round"></div>
					</label>
				</div>

				<div class="booking_options-flex-child bkap_help_class">
					<img class="help_tip" width="16" height="16" data-tip="<?php esc_attr_e( 'Enable this setting if the booking requires admin approval/confirmation. Payment will not be taken at Checkout', 'woocommerce-booking' ); ?>" src="<?php echo esc_url( plugins_url( 'woocommerce/assets/images/help.png' ) ); ?>" />
				</div>
		...
		public function bkap_remove_payment_methods( $available_gateways ) {
			...
			$this->bkap_load_requires_confirmation_payment_gateway();
								...
								<table class = "display_meta" cellspacing="0">
									<tbody>
										...
										foreach ( $booking_post_ids as $key => $booking_post_id ) {
											...
											<tr>
												<td>
												<a class="button" style="margin: 2px" href="<?php echo esc_url_raw( add_query_arg( $args, admin_url() . 'admin.php' ) ); ?>"><?php echo esc_html( $edit_booking_button ); ?></a><br>
												</td>
											</tr>
										<?php } ?>
			...
			if ( ! $check_if_renewal_booking ) {
				...
				if ( 'cancelled' === $old_status && in_array( $_status, array( 'paid', 'confirmed' ) ) ) {
					...
					$booking   = array(
						...
						'hidden_date'          => gmdate( 'd-m-Y', strtotime( $item_value['_wapbk_booking_date'] ) ),
						'date_checkout'        => $item_value['wapbk_checkout_date'],
						'hidden_date_checkout' => gmdate( 'd-m-Y', strtotime( $item_value['_wapbk_checkout_date'] ) ),
		...
		public static function update_booking_tables( $_status, $order_id, $item_id, $item_value, $order, $booking_id = null ) {
			...
			if ( 'cancelled' === $_status ) {
				...
				if ( ! empty( $booking_details ) ) {
					...
					Bkap_Zoom_Meetings::bkap_delete_zoom_meeting( $booking_post_id, $booking_post_obj );
				...
				foreach ( $new_booking_data as $data ) {
					Bkap_Zoom_Meetings::bkap_create_zoom_meeting( $booking_post_id, $data );
				}
				...
				if ( $order_has_multiple_bookings ) {
					...
					$wapbk_booking_date = gmdate( 'Y-m-d', strtotime( $_booking->get_start() ) );
				}
				...
				if ( $valid_date ) {
					...
					if ( in_array( $gcal->get_api_mode( $user_id, $item_value['product_id'] ), array( 'directly', 'oauth' ), true ) ) {
						...
						if ( $event_status ) {
							...
							/* Translators: %s Booking ID */
							$order_note = sprintf( __( 'Details of Booking #%s has been exported to Google Calendar', 'woocommerce-booking' ), (int) $booking_post_id );
		...
		public function bkap_can_be_cancelled_checkbox( $product_id, $booking_settings ) {
			...
			<div id="can_be_cancelled_section" class="booking_options-flex-main">
				...
				<div class="booking_options-flex-child bkap_help_class">
					<img class="help_tip" width="16" height="16" data-tip="<?php esc_attr_e( 'When enabled, this allows bookings to be cancelled by customers. Bookings can be cancelled anytime or at some duration before the booking.', 'woocommerce-booking' ); ?>" src="<?php echo esc_url( plugins_url( 'woocommerce/assets/images/help.png' ) ); ?>" />
				</div>
			</div>
