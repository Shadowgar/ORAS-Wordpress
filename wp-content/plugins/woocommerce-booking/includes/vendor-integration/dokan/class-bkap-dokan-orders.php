...
if ( ! class_exists( 'bkap_dokan_orders_class' ) ) {
	...
	class bkap_dokan_orders_class {
		...
		public function bkap_dokan_include_booking_styles() {
			...
			BKAP_Scripts::bkap_load_dokan_booking_styles( $plugin_version );
		}
		...
		public function bkap_download_booking_files() {
			...
			$view = isset( $_GET['view'] ) ? sanitize_text_field( wp_unslash( $_GET['view'] ) ) : ''; // phpcs:disable WordPress.Security.NonceVerification
			if ( $view === 'bkap-print' || $view === 'bkap-csv' ) {
				...
				if ( isset( $current_page ) && $current_page === 'bkap-csv' ) {
					BKAP_Vendor_Bookings::bkap_download_csv_file( $data );
				} elseif ( isset( $current_page ) && $current_page === 'bkap-print' ) {
					BKAP_Vendor_Bookings::bkap_download_print_file( $data );
				}
			}			
		}
		...
		public static function bkap_load_view_template( $booking_id, $booking_post, $booking_details, $item_no ) {
			...
			if ( isset( $booking_post['persons'] ) ) {
				$booking = new BKAP_Booking( $booking_id );
				$booking_details['persons'] = $booking->get_persons();
			}
			...
			wp_register_script(
				...
				BKAP_Files::rewrite_asset_url( '/assets/js/vendors/dokan/bkap-dokan-reschedule-booking.js', BKAP_FILE ),
				'',
				BKAP_VERSION,
				true
			);
		...
		public function bkap_dokan_change_status() {
			...
			$item_id     = isset( $_POST['item_id'] ) ? sanitize_text_field( wp_unslash( $_POST['item_id'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification
		...
		public function bkap_load_order_scripts() {
			...
			if ( dokan_is_seller_dashboard() && isset( $_GET['order_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
				wp_enqueue_style(
					'bkap_dokan_orders',
					BKAP_Files::rewrite_asset_url( '/assets/css/vendors/dokan/bkap-dokan-view-order.css', BKAP_FILE ),
					'',
					BKAP_VERSION,
					false
				);
