<?php
/**
 * REST API for resource objects.
 *
 * Handles requests to the /resources endpoint.
 *
 * @package WooCommerce-Booking\Resources\Rest\Controller
 */

/**
 * REST API Resources controller class.
 */
class BKAP_REST_API_BOOKING_RESOURCES_CONTROLLER {
	 ...
	 */
	protected $rest_base = 'bkap/resources';
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				'methods'             => WP_REST_Server::READABLE,
				'callback'            => array( $this, 'get_resources_count' ),
				'permission_callback' => array( $this, 'get_permission' ),
			)
		);
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( $this, 'bkap_get_resource' ),
					'permission_callback' => array( $this, 'get_permission' ),
				),
				array(
					'methods'             => WP_REST_Server::DELETABLE,
					'callback'            => array( $this, 'bkap_delete_resource' ),
					'permission_callback' => array( $this, 'get_permission' ),
				),
			)
		);
		...
		try {
			...
			if ( $this->check_if_exists( 'limit', $filter ) ) {
				if ( ! is_numeric( $filter['limit'] ) ) {
					return new WP_Error( 'bkap_api_resource_invalid_parameter', __( 'Limit parameter must be numeric', 'woocommerce-booking' ), array( 'status' => 400 ) );
				}
			}
			...
			if ( $this->check_if_exists( 'product_id', $filter ) ) {
				...
				if ( ! is_object( $post ) ) {
					return new WP_Error( 'bkap_api_resource_invalid_parameter', __( 'No valid Product has been found for the Product ID provided', 'woocommerce-booking' ), array( 'status' => 400 ) );
				}
			}

			if ( $this->check_if_exists( 'customer_id', $filter ) ) {
				if ( false === get_user_by( 'id', $filter['customer_id'] ) ) {
					return new WP_Error( 'bkap_api_resource_invalid_parameter', __( 'Customer ID is invalid', 'woocommerce-booking' ), array( 'status' => 400 ) );
				}
			}

			if ( $this->check_if_exists( 'quantity', $filter ) ) {
				if ( ! is_numeric( $filter['quantity'] ) ) {
					return new WP_Error( 'bkap_api_resource_invalid_parameter', __( 'Quantity parameter must be numeric', 'woocommerce-booking' ), array( 'status' => 400 ) );
				}
			}
	 ...
	 * @since 6.4.0
	 */
	public function bkap_get_resource( $request ) {

		$resource = $this->get_resource( (int) $request['id'] );
	...
	public function get_resource( $resource_id ) {
		...
		$id = $this->check_permission( $resource_id, 'read' );
		if ( is_wp_error( $id ) ) {
			return $id;
		}
	 ...
	 * @since 6.4.0
	 */
	public function bkap_delete_resource( $request ) {
		$resource = $this->delete_resource( (int) $request['id'] );
	...
	public function delete_resource( $resource_id ) {
		...
		try {
			...
			$id = $this->check_permission( $resource_id, 'delete' );
			...
			if ( false === get_post_status( $resource_id ) ) {
				return new WP_Error( 'bkap_api_resource_post_not_found', __( 'Resource cannot be found or does not exist', 'woocommerce-booking' ), array( 'status' => 400 ) );
			}
