		...
		public function get_products( $data = array() ) {
			...
			try {
				...
				if ( $this->check_if_exists( 'booking_type', $filter ) ) {
					...
					if ( array_diff( $filter['booking_type'], BKAP_API_Bookings::get_bookings_types( 'keys' ) ) ) {
						...
						throw new WC_API_Exception(
							...
							sprintf(
								...
								implode( ', ', array_keys( BKAP_API_Bookings::get_bookings_types( 'keys' ) ) )
							),
							400
						);
					}
				...
				if ( $this->check_if_exists( 'end_date', $filter ) ) {
					...
					if ( false === $date ) {
						throw new WC_API_Exception(
							'bkap_api_product_invalid_parameter',
							__( 'Please provide the End Date in the Y-m-d format. Ex: 2021-09-11', 'woocommerce-booking' ),
							400
						);
					}
				}
						...
						if ( is_array( $data['availability_dates_months'] ) ) {
							...
							foreach ( $data['availability_dates_months'] as $availability ) {
								...
								if ( 'specific_dates' === $availability['type'] ) {
									...
									$specific_dates['specific_dates_date'] = implode( ',', $dates );
		...
		public function update_booking_product( $id, $data ) {
			...
			try {
				...
				if ( is_array( $process_data ) && count( $process_data ) > 0 ) {
					...
					// TODO: Return error messages from the function above.
