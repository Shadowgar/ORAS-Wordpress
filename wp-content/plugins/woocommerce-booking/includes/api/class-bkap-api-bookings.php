		...
		public function get_bookings( $data = array() ) {
			...
			try {
				if ( ! empty( $data ) ) {
					...
					$filter = $_REQUEST; // phpcs:ignore WordPress.Security.NonceVerification
				}
			...
			try {
				...
				if ( $this->is_data_set( 'variation_id', $data ) ) {
					...
					if ( ! is_object( $variable ) ) {
						throw new WP_Error( 'bkap_api_booking_user_invalid_product', __( 'Variation is invalid for the Variation ID that has been provided', 'woocommerce-booking' ), 400 );
					}
