...
if ( ! class_exists( 'Bkap_Rescheduled_Order_Class' ) ) {
	...
	class Bkap_Rescheduled_Order_Class {
							...
							foreach ( $booking_ids as $b_id ) :
								...
								<tr class="order">
									...
									<td class="booking-details" data-title="<?php esc_attr_e( 'Booking Details', 'woocommerce-booking' ); ?>">
										<?php echo $booking_details; // phpcs:ignore ?>
									</td>
									<td class="booking-details" data-title="<?php esc_attr_e( 'Booking Details', 'woocommerce-booking' ); ?>">
										<?php
										...
										if ( 'cancelled' !== $booking->get_status() ) {
											printf( '<input type="button" class="bkap_edit_bookings" onclick="bkap_edit_booking_class.bkap_edit_bookings(%d,%s,%d)" value="%s">', esc_attr( $item->get_product_id( 'view' ) ), esc_attr( $item_id . '.' . $i ), esc_attr( $i ), esc_attr( $edit_booking_label ) );
										} else {
		...
		public static function bkap_rescheduled_create_order( $original_order_id, $item ) {
			...
			wc_update_order_item( $item_id, array( 'order_item_name' => sprintf( __( 'Additional Payment for %1$s (Order #%2$d )', 'woocommerce-booking' ), $item['product']->get_title(), $original_order_id ) ) );
			...
			if ( $item['_bkap_resch_rem_bal_order_id'] !== '' && $item['_bkap_resch_rem_bal_order_id'] !== null ) {
				...
				?>
					<a href="<?php echo esc_url( $order_url ); ?>" class="button button-small">
						<?php esc_html_e( 'Related Order', 'woocommerce-booking' ); ?>
					</a>
				<?php
				...
				?>
					<a href="<?php echo esc_url( $order_url ); ?>" class="button button-small">
						<?php esc_html_e( 'Parent Order', 'woocommerce-booking' ); ?>
					</a>
				<?php
			}
		}
