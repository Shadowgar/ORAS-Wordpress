...
if ( ! class_exists( 'bkap_variations' ) ) {
	...
	class bkap_variations {
			...
			if ( isset( $post->post_status ) && ( 'publish' === $post->post_status || 'draft' === $post->post_status ) ) {
				...
				<?php
					woocommerce_wp_textarea_input(
						array(
							'id'            => "variation_holidays_{$loop}",
							'name'          => "_holidays[{$loop}]",
							'value'         => esc_attr( get_post_meta( $variation->ID, '_holidays', true ) ),
							'label'         => __( 'Holidays', 'woocommerce-booking' ),
							'desc_tip'      => true,
							'description'   => __( 'Selected dates will be set as Holiday/Disabled/Block date for this variation.', 'woocommerce-booking' ),
							'wrapper_class' => 'form-row form-row-full',
							'class'         => 'input-text short variation_holidays',
						)
					);
			}
		}
		...
		public function save_variation_fields( $variation_id, $i ) {
			...
			if ( isset( $_POST['_holidays'] ) ) {
				$holidays = $_POST['_holidays'][ $i ];
				if ( isset( $holidays ) ) {
					update_post_meta( $variation_id, '_holidays', $holidays );
				}
			}
		...
		function print_hidden_lockout( $product_id ) {
			...
			if ( in_array( $product_type, array( 'variable', 'variable-subscription' ), true ) && isset( $booking_settings['booking_enable_date'] ) && 'on' === $booking_settings['booking_enable_date'] ) {
		...
		public function validate_multiple_days_product_page() {
			...
			if ( isset( $product_type ) && in_array( $product_type, array( 'variable', 'variable-subscription' ), true ) && isset( $variation_lockout ) && $variation_lockout > 0 ) {
				$_POST['validated'] = 'YES';
		...
		function validate_single_days_product_page() {
			...
			if ( isset( $product_type ) && in_array( $product_type, array( 'variable', 'variable-subscription' ), true ) && isset( $variation_lockout ) && $variation_lockout > 0 ) {
				...
				if ( $final_qty > $variation_lockout ) {
					...
					$date_to_display = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $booking_date ) );
		...
		function validate_date_time_product_page() {
			...
			if ( isset( $product_type ) && in_array( $product_type, array( 'variable', 'variable-subscription' ), true ) && isset( $variation_lockout ) && $variation_lockout > 0 ) {
				...
				$booking_date  = isset( $_POST['wapbk_hidden_date'] ) ? $_POST['wapbk_hidden_date'] : ''; // booking date.
				$booking_time  = isset( $_POST['time_slot'] ) ? $_POST['time_slot'] : ''; // booking time.
				...
				$final_qty = isset( $_POST['quantity'] ) ? $_POST['quantity'] : 0; // qty.
		...
		function validate_multiple_days_cart_page() {
			...
			if ( $variation_id > 0 && isset( $variation_lockout ) && $variation_lockout > 0 ) {
				...
				if ( is_array( $get_order_item_ids ) && count( $get_order_item_ids ) > 0 ) {
							...
							if ( ( $order_status != 'wc-cancelled' ) && ( $order_status != 'wc-refunded' ) && ( $order_status != 'wc-trash' ) && ( $order_status != 'wc-failed' ) && 'trash' !== $booking_status && 'cancelled' !== $booking_status ) {
								...
								if ( is_array( $get_dates ) && count( $get_dates ) > 0 ) {
									...
									if ( isset( $get_dates[1]->meta_value ) ) {
										$start_date = gmdate( 'j-n-Y', strtotime( $get_dates[1]->meta_value ) );
									}
									if ( isset( $get_dates[2]->meta_value ) ) {
										$end_date = gmdate( 'j-n-Y', strtotime( $get_dates[2]->meta_value ) );
									}
										...
										if ( isset( $start_date ) && $start_date != '' && isset( $end_date ) && $end_date != '' ) {
											...
											if ( function_exists( 'is_bkap_rental_active' ) && is_bkap_rental_active() ) {
												...
												if ( isset( $booking_settings['booking_prior_days_to_book'] ) && $booking_settings['booking_prior_days_to_book'] ) {
													...
													$prior_date  = gmdate( 'j-n-Y', strtotime( $days, strtotime( $start_date ) ) );
												...
												if ( isset( $booking_settings['booking_later_days_to_book'] ) && $booking_settings['booking_later_days_to_book'] ) {
													...
													$late_date    = gmdate( 'j-n-Y', strtotime( $days, strtotime( $end_date ) ) );
													$end_date_new = gmdate( 'j-n-Y', strtotime( '+1 day', strtotime( $end_date ) ) );
					...
					/* Consider Bookings without orders */
					$bookings_without_order = "
						SELECT pm1.post_id 
						FROM {$wpdb->prefix}postmeta pm1
						INNER JOIN {$wpdb->prefix}postmeta pm2 
						ON pm1.post_id = pm2.post_id
						WHERE pm1.meta_key = '_bkap_variation_id' 
						AND pm1.meta_value = %d
						AND pm2.meta_key = '_bkap_parent_id' 
						AND pm2.meta_value = 0";
						// phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL
					$booking_ids_without_order = $wpdb->get_results( $wpdb->prepare( $bookings_without_order, $variation_id ), ARRAY_A );

					if ( count( $booking_ids_without_order ) > 0 ) {
						$booking_type = bkap_type( $product_id );
						foreach ( $booking_ids_without_order as $booking_ids ) {
							$booking_id = $booking_ids['post_id'];
							...
							switch ( $booking_type ) {
								...
								case 'only_day':
									if ( function_exists( 'is_bkap_rental_active' ) && is_bkap_rental_active() ) {
										if ( isset( $booking_settings['booking_prior_days_to_book'] ) && $booking_settings['booking_prior_days_to_book'] ) {
											$days       = '-' . $booking_settings['booking_prior_days_to_book'] . ' days';
											$start_date = gmdate( 'j-n-Y', strtotime( $days, strtotime( $start_date ) ) );
										}

										if ( isset( $booking_settings['booking_later_days_to_book'] ) && $booking_settings['booking_later_days_to_book'] ) {
											$days     = '+' . $booking_settings['booking_later_days_to_book'] . ' days';
											$end_date = gmdate( 'j-n-Y', strtotime( $days, strtotime( $end_date ) ) );
										}
									}

									if ( $start_date === $end_date ) {
										$booked_date_range = array( gmdate( 'j-n-Y', strtotime( $start_date ) ) );
									} else {
										$booked_date_range = bkap_common::bkap_get_betweendays( $start_date, $end_date, 'j-n-Y' );
									}

									$bkap_qty = $booking->get_quantity();

									foreach ( $booked_date_range as $key => $value ) {
										if ( array_key_exists( $value, $total_bookings ) ) {
											$qty_present              = $total_bookings[ $value ];
											$new_qty                  = $qty_present + $bkap_qty;
											$total_bookings[ $value ] = $new_qty;
										} else {
											$total_bookings[ $value ] = $bkap_qty;
										}
									}
									break;
				...
				if ( isset( $check ) && 'failed' == $check ) {
					if ( is_array( $date_availablity ) && count( $date_availablity ) > 0 ) {
						...
						// setup the dates to be displayed
						$check_in_to_display  = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $date_checkin ) );
		...
		function validate_single_days_cart_page() {
			...
			$quantity_booked = $_POST['quantity'];
			$booking_date    = gmdate( 'j-n-Y', strtotime( $booking_date ) );
						...
						if ( $order && false !== $booking_status ) {
							...
							if ( ( $order_status != 'wc-cancelled' ) && ( $order_status != 'wc-refunded' ) && ( $order_status != 'wc-trash' ) && ( $order_status != 'wc-failed' ) && 'trash' !== $booking_status && 'cancelled' !== $booking_status ) {
								...
								if ( is_array( $get_dates ) && count( $get_dates ) > 0 ) {
									...
									$dates       = array();
									if ( isset( $get_dates[1]->meta_value ) ) {
										$booked_date = gmdate( 'j-n-Y', strtotime( $get_dates[1]->meta_value ) );
									}
					...
					foreach ( $get_order_item_ids as $item_key => $item_value ) {
						...
						if ( $order && false !== $booking_status ) {
							...
							if ( isset( $order_status ) && ( $order_status != '' ) && ( $order_status != 'wc-cancelled' ) && ( $order_status != 'wc-refunded' ) && ( $order_status != 'wc-trash' ) && ( $order_status != 'wc-failed' ) && 'trash' !== $booking_status && 'cancelled' !== $booking_status ) {
								...
								$current_time = current_time( 'timestamp' );
								$current_time = gmdate( 'Y-m-d', $current_time );
									...
									if ( is_array( $get_dates ) && count( $get_dates ) > 0 ) {
										...
										if ( isset( $start_date ) && $start_date != '' && isset( $end_date ) && $end_date != '' ) {
											...
											if ( strtotime( $start_date ) > strtotime( $end_date ) ) {
												...
												$start_date = gmdate( 'j-n-Y', strtotime( $get_dates[2]->meta_value ) );
												$end_date   = gmdate( 'j-n-Y', strtotime( $get_dates[1]->meta_value ) );
											}
										}
									...
									if ( is_array( $get_dates ) && count( $get_dates ) > 0 ) {
										...
										if ( $current_time <= strtotime( $booked_date ) ) {
											...
											if ( function_exists( 'is_bkap_rental_active' ) && is_bkap_rental_active() ) {
												...
												if ( isset( $booking_settings['booking_prior_days_to_book'] ) && $booking_settings['booking_prior_days_to_book'] ) {
													$days        = '-' . $booking_settings['booking_prior_days_to_book'] . ' days';
													$prior_date  = gmdate( 'j-n-Y', strtotime( $days, strtotime( $booked_date ) ) );
													$first_date  = $prior_date;
												...
												if ( isset( $booking_settings['booking_later_days_to_book'] ) && $booking_settings['booking_later_days_to_book'] ) {
													$days         = '+' . $booking_settings['booking_later_days_to_book'] . ' days';
													$late_date    = gmdate( 'j-n-Y', strtotime( $days, strtotime( $booked_date ) ) );
													$end_date_new = gmdate( 'j-n-Y', strtotime( '+1 day', strtotime( $booked_date ) ) );
													$later_block  = bkap_common::bkap_get_betweendays( $end_date_new, $late_date );
							...
							if ( isset( $list_dates ) && is_array( $list_dates ) && count( $list_dates ) > 0 ) {
								...
								foreach ( $list_dates as $date_key => $qty_value ) {
									...
									$date_key = gmdate( 'j-n-Y', strtotime( $date_key ) );
