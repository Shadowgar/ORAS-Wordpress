...
if ( ! class_exists( 'Bkap_Validation' ) ) {
	...
	class Bkap_Validation {
		...
		public static function bkap_validate_overlap_time_cart( $product_id, $post, $check_in_date, $from_time_slot, $to_time_slot ) {
			...
			$check_in_date_jny      = gmdate( 'j-n-Y', strtotime( $check_in_date ) );
						...
						foreach ( WC()->cart->cart_contents as $prod_in_cart_key => $prod_in_cart_value ) {
							...
							if ( isset( $prod_in_cart_value['bkap_booking'] ) && ! empty( $prod_in_cart_value['bkap_booking'] ) ) {
								...
								foreach ( $booking_data as $value ) {
									...
									if ( isset( $value['time_slot'] ) && '' !== $value['time_slot'] && $check_in_date_jny === $value['hidden_date'] ) {
		...
		public static function bkap_get_quantity( $post_id, $bundle_child_qty = '' ) {
			...
			if ( isset( $_POST['wapbk_hidden_date'] ) && '' !== $_POST['wapbk_hidden_date'] ) {
				...
				$date_to_display = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $_POST['wapbk_hidden_date'] ) ); // Date as per global setting.
			}
			...
			if ( isset( $booking_settings['booking_product_holiday'] ) && ! empty( $booking_settings['booking_product_holiday'] ) ) {
				foreach ( $date_checks as $key => $value ) {
					if ( isset( $booking_settings['booking_product_holiday'][ $value ] ) ) {
						$booking_date = $booking_dates[ $key ];
						$message = sprintf(
							/* translators: %1s: PRoduct Name; %2s: Date Information. */
							__( '%1$s cannot be booked for %2$s due to holiday. Please choose some other date for booking.', 'woocommerce-booking' ),
							$product_name,
							$booking_date,
						);
			...
			switch ( $booking_type ) {
				...
				case 'multidates':
					...
					if ( ! isset( $_POST['validated'] ) || ( isset( $_POST['validated'] ) && $_POST['validated'] == 'NO' ) ) {
						...
						foreach ( $date_checks as $date_check ) { // loop through each date.
							...
							$date_check_ymd = gmdate( 'Y-m-d', strtotime( $date_check ) );
							...
							if ( isset( $results ) && count( $results ) > 0 ) {
								...
								$date_to_display = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $results[0]->start_date ) );
				...
				case 'multiple_days':
					...
					if ( ! isset( $_POST['validated'] ) || ( isset( $_POST['validated'] ) && $_POST['validated'] == 'NO' ) ) {
						...
						$date_checkout   = gmdate( 'd-n-Y', strtotime( $_POST['wapbk_hidden_date_checkout'] ) );
						$date_cheeckin   = gmdate( 'd-n-Y', strtotime( $_POST['wapbk_hidden_date'] ) );
						if ( isset( $booking_settings['booking_prior_days_to_book'] ) && $booking_settings['booking_prior_days_to_book'] ) {
							...
							$date_cheeckin = date( 'j-n-Y', strtotime( $days, strtotime( $date_cheeckin ) ) );
						}
						if ( isset( $booking_settings['booking_later_days_to_book'] ) && $booking_settings['booking_later_days_to_book'] ) {
							...
							$date_checkout = date( 'j-n-Y', strtotime( $days, strtotime( $date_checkout ) ) );
						}
						$order_dates     = bkap_common::bkap_get_betweendays( $date_cheeckin, $date_checkout, 'j-n-Y' );
						$dates_new_array = bkap_get_booked( $post_id, false, true );
						$dates_new_array = $dates_new_array['dates'];
						$dates_new_arr   = array();

						foreach ( $dates_new_array as $key => $value ) {
							$date_jny                   = gmdate( 'j-n-Y', strtotime( $key ) );
							$dates_new_arr[ $date_jny ] = $value;
						}
						...
						if ( $main_lockout > 0 ) {
							...
							if ( $quantity_check_pass == 'no' ) {
								...
								if ( is_array( $date_availablity ) && count( $date_availablity ) > 0 ) {
									...
									$check_in_to_display  = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $_POST['wapbk_hidden_date'] ) );
									$check_out_to_display = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $_POST['wapbk_hidden_date_checkout'] ) );
						...
						if ( $quantity_check_pass == 'yes' ) {
							...
							foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
								...
								if ( isset( $booking[0]['hidden_date'] ) && isset( $booking[0]['hidden_date_checkout'] ) ) {
									$hidden_date          = gmdate( 'd-n-Y', strtotime( $booking[0]['hidden_date'] ) );
									$hidden_date_checkout = gmdate( 'd-n-Y', strtotime( $booking[0]['hidden_date_checkout'] ) );
									$dates                = bkap_common::bkap_get_betweendays( $hidden_date, $hidden_date_checkout, 'j-n-Y' );
								}
								...
								if ( $product_id == $post_id ) {
									...
									if ( $main_lockout > 0 ) {
										...
										foreach ( $order_dates as $k => $v ) {
											...
											$date_to_display = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $v ) );
							...
							foreach ( $date_checks as $date_check_key => $date_check ) { // loop through each date.
								...
								if ( $overlapping ) {
									...
									if ( ! empty( $overlapping_quantity_check_pass ) ) {
										...
										if ( $time_format === '12' ) {
											$overlap_start_time = gmdate( 'h:i A', strtotime( $overlap_time[0] ) );
											$overlap_end_time   = gmdate( 'h:i A', strtotime( $overlap_time[1] ) );
										} else {
											$overlap_start_time = gmdate( 'H:i', strtotime( $overlap_time[0] ) );
											$overlap_end_time   = gmdate( 'H:i', strtotime( $overlap_time[0] ) );
										}

										$overlap_date_to_display = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $overlapping_quantity_check_pass['pass_fail_date'] ) );
								...
								if ( $to_time != '' ) {
									...
									if ( isset( $results ) && count( $results ) == 0 ) {
										$db_from_time = gmdate( 'H:i', strtotime( $db_from_time ) );
										$db_to_time   = gmdate( 'H:i', strtotime( $db_to_time ) );
										$results      = $wpdb->get_results( $wpdb->prepare( $query, $post_id, $date_check_ymd, $db_from_time, $db_to_time ) );
									}

									if ( isset( $results ) && count( $results ) == 0 ) {

										$weekday         = gmdate( 'w', strtotime( $date_check_ymd ) );
									...
									if ( isset( $results ) && count( $results ) == 0 ) {
										$f_t     = gmdate( 'H:i', strtotime( $db_from_time ) );
								...
								if ( isset( $results ) && count( $results ) > 0 ) {
									...
									if ( isset( $time_slots[ $date_check_key ] ) && $time_slots[ $date_check_key ] != '' ) {
										...
										// $date_to_display = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $date_check ) );
				...
				foreach ( $wc_cart_object->cart->cart_contents as $key => $value ) {
					...
					foreach ( $bkap_bookings as $b => $bkap_booking ) { // Multidates mate nu changes.
						...
						if ( isset( $bkap_booking['date_checkout'] ) ) {
							...
							$check_out_to_display = gmdate( $bkap_date_formats[ $date_format_to_display ], strtotime( $date_checkout ) );
						}
						...
						if ( isset( $bkap_booking['resource_id'] ) && '' !== $bkap_booking['resource_id'] ) {
							...
							$post_data = array(
								...
								'quantity'             => isset( $_POST['quantity'] ) ? sanitize_text_field( wp_unslash( $_POST['quantity'] ) ) : 1, // phpcs:ignore WordPress.Security.NonceVerification
								'resource_id'          => $bkap_booking['resource_id'],
								'wapbk_hidden_date'    => $date_check,
								'time_slot'            => isset( $bkap_booking['time_slot'] ) ? $bkap_booking['time_slot'] : '',
								'duration_time_slot'   => isset( $bkap_booking['duration_time_slot'] ) ? $bkap_booking['duration_time_slot'] : '',
								'hidden_date_checkout' => $date_checkout,
								'hidden_date'          => $bkap_booking['hidden_date'],
							);

							if ( '' !== $post_data['duration_time_slot'] ) {
								$bkap_duration_field              = explode( '-', $bkap_booking['selected_duration'] );
								$post_data['bkap_duration_field'] = $bkap_duration_field[0];
							}
						...
						switch ( $booking_type ) {
										...
										if ( $validation_completed == 'NO' ) {
											...
											if ( ! $results ) {
												...
												if ( count( $bookings_data ) > 0 ) {
													if ( isset( $bookings_data[ gmdate( 'Ymd', strtotime( $date_check ) ) ] ) ) {
														$db_to_time = ( '' === $db_to_time ) ? '00:00' : $db_to_time;
														if ( isset( $bookings_data[ gmdate( 'Ymd', strtotime( $date_check ) ) ][ $db_from_time . ' - ' . $db_to_time ] ) ) {
															$available = $bookings_data[ gmdate( 'Ymd', strtotime( $date_check ) ) ][ $db_from_time . ' - ' . $db_to_time ];
							...
							case 'duration_time':
								if ( isset( $bkap_booking['duration_time_slot'] ) && $bkap_booking['duration_time_slot'] != '' ) {
									...
									foreach ( $wc_cart_object->cart->cart_contents as $cart_check_key => $cart_check_value ) {
										...
										if ( $value['product_id'] == $cart_check_value['product_id'] && isset( $cart_check_value['bkap_booking'] )
										&& $bkap_booking['hidden_date'] == $cart_check_value['bkap_booking'][0]['hidden_date']
							...
							case 'multiple_days':
								...
								$hidden_date          = $bkap_booking['hidden_date'];
								$hidden_date_checkout = $bkap_booking['hidden_date_checkout'];

								if ( isset( $booking_settings['booking_prior_days_to_book'] ) && $booking_settings['booking_prior_days_to_book'] ) {
									$days      = '-' . $booking_settings['booking_prior_days_to_book'] . ' days';
									$hidden_date = gmdate( 'j-n-Y', strtotime( $days, strtotime( $hidden_date ) ) );
								}
								if ( isset( $booking_settings['booking_later_days_to_book'] ) && $booking_settings['booking_later_days_to_book'] ) {
									$days    = '+' . $booking_settings['booking_later_days_to_book'] . ' days';
									$hidden_date_checkout = gmdate( 'j-n-Y', strtotime( $days, strtotime( $hidden_date_checkout ) ) );
								}
								...
								foreach ( $wc_cart_object->cart->cart_contents as $cart_check_key => $cart_check_value ) {
									...
									if ( $value['product_id'] == $cart_check_value['product_id'] ) {
										...
										$cart_hidden_date          = $cart_check_value['bkap_booking'][0]['hidden_date'];
										$cart_hidden_date_checkout = $cart_check_value['bkap_booking'][0]['hidden_date_checkout'];

										if ( isset( $booking_settings['booking_prior_days_to_book'] ) && $booking_settings['booking_prior_days_to_book'] ) {
											$days      = '-' . $booking_settings['booking_prior_days_to_book'] . ' days';
											$cart_hidden_date = gmdate( 'j-n-Y', strtotime( $days, strtotime( $cart_hidden_date ) ) );
										}
										if ( isset( $booking_settings['booking_later_days_to_book'] ) && $booking_settings['booking_later_days_to_book'] ) {
											$days    = '+' . $booking_settings['booking_later_days_to_book'] . ' days';
											$cart_hidden_date_checkout = gmdate( 'j-n-Y', strtotime( $days, strtotime( $cart_hidden_date_checkout ) ) );
										}

										if ( isset( $cart_check_value['bkap_booking'] ) && bkap_date_ranges_overlapping( $hidden_date, $hidden_date_checkout, $cart_hidden_date, $cart_hidden_date_checkout ) && $key != $cart_check_key ) {
								...
								$qty_check = ( 0 === $qty_check ) ? ( $value['quantity'] * $total_person ) : $qty_check;

								// phpcs:ignore WordPress.Security.NonceVerification
								$validation_completed = isset( $_POST['validation_status'] ) ? sanitize_text_field( wp_unslash( $_POST['validation_status'] ) ) : '';

								if ( 'NO' === $validation_completed ) {
									$order_dates     = bkap_common::bkap_get_betweendays( $date_cheeckin, $date_checkout, 'j-n-Y' );
									$dates_new_array = bkap_get_booked( $duplicate_of, false, true );
									$dates_new_array = $dates_new_array['dates'];
							...
							case 'multidates':
								...
								if ( empty( $validation_completed ) || $validation_completed == 'NO' ) {
									...
									$lockout = get_date_lockout( $value['product_id'], $date_check, false );
									if ( 'unlimited' !== $lockout ) {
										...
										if ( count( $bookings_data ) > 0 ) {
											if ( isset( $bookings_data[ gmdate( 'Ymd', strtotime( $date_check ) ) ] ) ) {
												$available = $bookings_data[ gmdate( 'Ymd', strtotime( $date_check ) ) ];
							...
							if ( isset( $value['hidden_date'] ) && $value['hidden_date'] != '' ) { // can be blanks if the product has been purchased without a date
								...
								if ( isset( $value['time_slot'] ) && $value['time_slot'] != '' ) {
									...
									foreach ( $timeslots as $k => $v ) {
										if ( '' !== $v ) {
											...
											$dateymd       = gmdate( 'Y-m-d', $date_strtotime );
								...
								if ( count( $custom_ranges ) > 0 ) {
									...
									foreach ( $custom_ranges as $r_key => $range_value ) {
										...
										$dates[]          = gmdate( 'd-n-Y', strtotime( $range_value['end'] ) ); // adding last date.
									}

									$date = gmdate( 'd-n-Y', strtotime( $date ) );
		...
		public static function bkap_same_bookings_in_cart_validation( $passed, $product_id, $booking_settings, $product ) {
			...
			if ( isset( $_POST['page_type'] ) && ( in_array( $_POST['page_type'], array( 'cart', 'checkout' ) ) ) ) { // phpcs:ignore
			...
			if ( $passed && isset( $_POST['wapbk_hidden_date'] ) ) {
				...
				if ( ! empty( $bkap_booking ) ) {
					...
					if ( ! $passed ) {
						$message = apply_filters( 'bkap_message_for_same_bookings_in_cart', $message, $product_id );
						wc_add_notice( $message, $notice_type = 'error' );
					}
				}
			}
