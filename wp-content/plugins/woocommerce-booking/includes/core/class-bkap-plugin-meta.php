...
if ( ! class_exists( 'Bkap_Plugin_Meta' ) ) {
	...
	class Bkap_Plugin_Meta {
		...
		public function __construct() {
			...
			add_filter( 'wp_plugin_check_checks', array( &$this, 'bkap_plugin_check_ignore_files' ), 10, 2 );
		}

		/**
		 * Fucntion to ignore the things from Plugin Check Plugin.
		 *
		 * @param mixed $checks Array of options to be considered for plugin check.
		 */
		public function bkap_plugin_check_ignore_files( $checks ) {
		...
		public static function bkap_plugin_row_meta( $links, $file ) {
			...
			if ( $file === $plugin_base_name ) {
				...
				$row_meta = array(
					'docs'    => '<a href="' . esc_url( apply_filters( 'woocommerce_booking_and_appointment_docs_url', 'https://www.tychesoftwares.com/docs/docs/booking-appointment-plugin-for-woocommerce-new/' ) ) . '" title="' . esc_attr( __( 'View Booking & Appointment Plugin Documentation', 'woocommerce-booking' ) ) . '">' . __( 'Docs', 'woocommerce-booking' ) . '</a>',
					'support' => '<a href="' . esc_url( apply_filters( 'woocommerce_booking_and_appointment_support_url', 'https://support.tychesoftwares.com/help/2285384554' ) ) . '" title="' . esc_attr( __( 'Submit Ticket', 'woocommerce-booking' ) ) . '">' . __( 'Submit Ticket', 'woocommerce-booking' ) . '</a>',
		...
		public function bkap_plugin_settings_link( $links ) {
			$setting_link['settings'] = '<a href="' . esc_url( get_admin_url( null, 'admin.php?page=bkap_page&action=settings#/' ) ) . '">Settings</a>';
