...
if ( ! class_exists( 'Bkap_Plugin_Update' ) ) {
	...
	class Bkap_Plugin_Update {
		...
		public function __construct() {
			...
			add_action( 'admin_init', array( &$this, 'bkap_add_transients' ) ); // Add a transients to handle display of notices based on it.
		}
		...
		public function bkap_display_timeslot_notices() {
			...
			if ( ( false !== $bkap_timeslot_notice ) && ( '1' === $bkap_timeslot_notice ) ) {
				...
				$message = sprintf( __( 'Introducing List View for Time Slots for Bookable products in the Booking & Appointment plugin for WooCommerce. Configure from <a href="%s">here</a>.', 'woocommerce-booking' ), $url ); // phpcs:ignore
