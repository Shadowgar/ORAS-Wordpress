...
if ( ! class_exists( 'bkap_addon_compatibility_class' ) ) {
	...
	class bkap_addon_compatibility_class {
		...
		public static function bkap_final_price_json_data_alg( $wp_send_json, $product_id ) {
			...
			$alg_lang = isset( $_POST['alg_lang'] ) ? sanitize_text_field( wp_unslash( $_POST['alg_lang'] ) ) : ''; // phpcs:ignore

			if ( $alg_lang ) {

				$price     = $wp_send_json['total_price_calculated'];
				$alg_price = alg_get_product_price_by_currency( $price, $alg_lang, wc_get_product( $product_id ) );
				...
				$wc_price_args['currency']  = $alg_lang;
		...
		public function bkap_add_composite_child_meta( $cart_item_meta ) {
			...
			if ( array_key_exists( 'composite_parent', $cart_item_meta ) && $cart_item_meta['composite_parent'] !== '' && isset( WC()->cart->cart_contents[ $cart_item_meta['composite_parent'] ]['bkap_booking'] ) ) {
				...
				if ( isset( $component_data['priced_individually'] ) && 'yes' === $component_data['priced_individually'] ) {
					if ( isset( $composite_data['variation_id'] ) && $composite_data['variation_id'] !== '' ) {
						...
						if ( isset( $_POST['wapbk_diff_days'] ) ) {
							$wapbk_diff_days = sanitize_text_field( wp_unslash( $_POST['wapbk_diff_days'] ) );
							if ( '' === $wapbk_diff_days ) {
								$cart_arr['price'] = $cart_arr['price'];
							} else {
								$cart_arr['price'] = $cart_arr['price'] * (int) $wapbk_diff_days;
							}
						}
					} else {
						...
						if ( ! empty( $composite_parent_booking ) ) {
							if ( 'multiple_days' === $booking_type ) {
								$param = array(
									gmdate( 'Y-m-d', strtotime( $composite_parent_booking['hidden_date'] ) ),
									gmdate( 'Y-m-d', strtotime( $composite_parent_booking['hidden_date_checkout'] ) ),
									$booking_type,
								);
							} else {
								$param = array(
									gmdate( 'Y-m-d', strtotime( $composite_parent_booking['hidden_date'] ) ),
									gmdate( 'w', strtotime( $composite_parent_booking['hidden_date'] ) ),
									$booking_type,
								);
							}
						}
						...
						if ( isset( $_POST['wapbk_diff_days'] ) && $_POST['wapbk_diff_days'] > 0 ) {
							$wapbk_diff_days   = isset( $_POST['wapbk_diff_days'] ) ? sanitize_term_field( wp_unslash( $_POST['wapbk_diff_days'] ) ) : 1;
							$cart_arr['price'] = $price * (int) $wapbk_diff_days;
						} else {
