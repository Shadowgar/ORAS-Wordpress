...
if ( ! class_exists( 'BKAP_Personal_Data_Export' ) ) {
	...
	class BKAP_Personal_Data_Export {
		 ...
		 * @since 4.9.0
		 */
		public static function bkap_data_exporter( $email_address, $page ) {
			global $wpdb;
		 ...
		 * @since 4.9.0
		 */
		public static function bkap_gcal_data_exporter( $email_address, $page ) {
			...
			foreach ( $booking_details_to_export as $prop => $name ) {
				...
				switch ( $prop ) {
					...
					case 'booking_details':
						...
						$start = gmdate( $date_format, $booking->start );
						$end   = gmdate( $date_format, $booking->end );
						$value = $start . '<br>' . $end;

						break;
					case 'booking_created':
						$value = gmdate( $date_format . ' ' . $time_format, strtotime( $booking->booking_date ) );
		 ...
		 * @since 4.9.0
		 */
		public static function get_gcal_event_data( $google_id ) {
			$personal_data = array();
			...
			$google = new BKAP_Google_Calendar_Event( $google_id );
