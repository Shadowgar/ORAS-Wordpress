...
class BKAP_Google_Calendar {
	...
	/**
	 * BKAP_Google_Client object.
	 *
	 * @var obj $client BKAP_Google_Client obj.
	 */
	public $client;

	/**
	 * Mode of Google Calendar Sync.
	 *
	 * @var string $sync_mode Mode of Google Calendar Sync.
	 */
	public $sync_mode;
	 ...
	 *
	 * @var obj $event Google Calendar Event.
	 */
	public $event;

	/**
	 * BKAP_OAuth_Google_Calendar service object.
	 *
	 * @var obj $service BKAP_OAuth_Google_Calendar service object.
	 */
	public $service;
	...
	public function __construct( $key_file = '', $service_account = '', $calendar = '' ) {
		...
		add_action( 'wp_ajax_bkap_test_connection_msg', array( __CLASS__, 'bkap_test_connection_msg' ) );
	}
		...
		if ( ! headers_sent() ) {
			wp_safe_redirect(
				esc_url(
					add_query_arg(
						array(
							'gcal_api_test_result' => rawurlencode( $exception ),
	 ...
	 * @since 2.6
	 */
	public function bkap_test_connection_msg( $data = array() ) {
		$error   = false;
		$message = '';
		...
		$is_gcal_api_test = isset( $_POST['gcal_api_test'] ) && 1 === $_POST['gcal_api_test'];
		$is_gcal_api_test = isset( $data['gcal_api_test'] ) ? 1 === $data['gcal_api_test'] : $is_gcal_api_test;
		...
		if ( $is_gcal_api_test ) {
			...
			if ( '' != $result ) {
				...
				if ( $result ) {
					$message .= sprintf(
						'<div class="notice notice-success"><p><b>%s</b> %s</p></div>',
						esc_html__( 'Test is successful', 'woocommerce-booking' ),
						esc_html__( 'Please REFRESH your Google Calendar and check that the test appointment has been saved.', 'woocommerce-booking' )
					);
				} else {
					$log_path = $this->uploads_dir . 'bkap-log.txt';
					$message .= sprintf(
						'<div class="notice notice-error"><p><b>%s</b>. %s</p></div>',
						esc_html__( 'Test failed', 'woocommerce-booking' ),
						sprintf(
							/* Translators: %s Log file path */
							esc_html__( 'Please inspect your log located at %s for more info.', 'woocommerce-booking' ),
							esc_html( $log_path )
						)
					);
				}
		...
		if ( isset( $_POST['gcal_api_test_result'] ) && '' != $_POST['gcal_api_test_result'] ) {
			$m = stripslashes( urldecode( sanitize_text_field( wp_unslash( $_POST['gcal_api_test_result'] ) ) ) );
			...
			$message = sprintf(
				/* translators: %s: Error message from Google Calendar API */
				esc_html__( 'The following error has been reported by Google Calendar API: %s', 'woocommerce-booking' ),
				'<br />' . esc_html( $m ) . '<br />'
			);
			$message .= sprintf(
				/* translators: %s: Additional recommendation message */
				'<b>%s</b> %s',
				esc_html__( 'Recommendation:', 'woocommerce-booking' ),
				esc_html__( 'Please double-check your settings.', 'woocommerce-booking' ) . esc_html( $add )
			);
		}
	 ...
	 * @since 2.6
	 */
	public function get_service_account( $user_id, $product_id ) {
	 ...
	 * @since 5.19.0
	 */
	public function get_key_json_file( $user_id, $product_id ) {
	 ...
	 * @since 2.6
	 */
	public function get_key_file( $user_id, $product_id ) {
	 ...
	 * @since 2.6
	 */
	public function is_not_suitable( $user_id, $product_id ) {
		...
		if ( false && memory_get_usage() < 31000000 ) {
			return sprintf(
				...
				sprintf(
					/* translators: %s: Link to WordPress Codex for increasing memory limit */
					esc_html__(
						'Google PHP API Client requires at least 32 MByte Server RAM. Please check this link on how to increase it: %s',
						'woocommerce-booking'
					),
					'<a href="http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP" target="_blank">' .
					esc_html__( 'Increasing memory allocated to PHP', 'woocommerce-booking' ) .
					'</a>'
				)
			);
		}
		...
		if ( isset( $_POST['gcal_api_pre_test'] ) && 1 == $_POST['gcal_api_pre_test'] ) { // phpcs:ignore WordPress.Security.NonceVerification
			return sprintf(
				'<div class="notice notice-error"><p>%s</p></div>',
				esc_html__( 'Your server installation meets requirements.', 'woocommerce-booking' )
			);
		}

		if ( ! $this->_file_exists( $user_id, $product_id ) ) {
			return '<div class="notice notice-error"><p><b>' . esc_html__( 'Key file data does not exist. Please upload the key file in JSON format.', 'woocommerce-booking' ) . '</b></p></div>';
		}
	...
	function _file_exists( $user_id, $product_id ) {
		if ( '' != $this->get_key_json_file( $user_id, $product_id, 'private_key' ) ) {
			...
			return true;
		} elseif ( file_exists( BKAP_BOOKINGS_INCLUDE_PATH . 'gcal/key/' . $this->get_key_file( $user_id, $product_id ) . '.p12' ) ) {
			return true;
		} else {
			return false;
		}
	...
	function _file_get_contents( $user_id, $product_id ) {
		if ( '' != $this->get_key_json_file( $user_id, $product_id, 'private_key' ) ) {
			...
			return @file_get_contents( BKAP_BOOKINGS_INCLUDE_PATH . 'gcal/key/' . $this->get_key_file( $user_id, $product_id ) . '.p12' );
		} else {
			return '';
		}
	}
	...
	public function bkap_google_client( $user_id, $product_id ) {
		...
		BKAP_Files::include_file( BKAP_PLUGIN_PATH . '/includes/libraries/google/vendor/Client.php' );
	...
	public function set_event_parameters( $app ) {
		...
		$description = str_replace(
			...
			array( get_bloginfo( 'name' ), $app->client_name, $app->product, $app->product_with_qty, $app->order_date_time, $app->order_date, $app->id, $app->order_total, $app->client_phone, $app->order_note, $app->client_address, $app->client_email, $app->resource, $app->persons, $app->zoom_meeting, $app->client_city, $app->client_country ),
		...
		if ( $app->start_time == '' && $app->end_time == '' ) {
			$start->setDate( gmdate( 'Y-m-d', strtotime( $app->start ) ) );
			$end->setDate( gmdate( 'Y-m-d', strtotime( $app->end ) ) );
		} elseif ( $app->end_time == '' ) {
	...
	public function insert_event( $event_details, $event_id, $user_id, $product_id = 0, $test = false, $item_number = -1 ) {
		...
		if ( $test ) {
			$bkap             = new stdClass();
			$bkap->start      = gmdate( 'Y-m-d', $this->local_time );
			$bkap->end        = gmdate( 'Y-m-d', $this->local_time );
			$bkap->start_time = gmdate( 'H:i:s', $this->local_time + 600 );
			$bkap->end_time   = gmdate( 'H:i:s', $this->local_time + 2400 );
	...
	function gcal( $bkap, $user_type ) {
		...
		if ( $bkap->start_time == '' && $bkap->end_time == '' ) {
			...
			$gmt_start = get_gmt_from_date( gmdate( 'Y-m-d H:i:s', $start ), 'Ymd\THis\Z' );
			$gmt_end   = get_gmt_from_date( gmdate( 'Y-m-d H:i:s', $end ), 'Ymd\THis\Z' );
		} else {
			$start = strtotime( $bkap->start . ' ' . $bkap->start_time );
			...
			if ( $user_type == 'admin' ) {
				...
				if ( '' !== Bkap_Timezone_Conversion::get_timezone_var( 'bkap_timezone_name' ) ) {
					...
					$sstart = new DateTime( gmdate( 'Y-m-d H:i:s', $start ), new DateTimeZone( Bkap_Timezone_Conversion::get_timezone_var( 'bkap_timezone_name' ) ) );
					$sstart->setTimezone( new DateTimeZone( 'UTC' ) );
					$sstart->format( 'Ymd\THis\Z' );
					$gmt_start = $sstart->format( 'Ymd\THis\Z' );

					$eend = new DateTime( gmdate( 'Y-m-d H:i:s', $end ), new DateTimeZone( Bkap_Timezone_Conversion::get_timezone_var( 'bkap_timezone_name' ) ) );
	...
	function other_cal( $bkap, $user_type ) {
		...
		$description = str_replace(
			array( 'SITE_NAME', 'CLIENT', 'PRODUCT_NAME', 'PRODUCT_WITH_QTY', 'ORDER_DATE_TIME', 'ORDER_DATE', 'ORDER_NUMBER', 'PRICE', 'PHONE', 'NOTE', 'ADDRESS', 'EMAIL', 'RESOURCE', 'PERSONS', 'ZOOM_MEETING', 'CITY', 'COUNTRY' ),
			array( get_bloginfo( 'name' ), $bkap->client_name, $bkap->product, $bkap->product_with_qty, $bkap->order_date_time, $bkap->order_date, $bkap->id, $bkap->order_total, $bkap->client_phone, $bkap->order_note, $bkap->client_address, $bkap->client_email, $bkap->resource, $bkap->persons, rawurlencode( $bkap->zoom_meeting ), $bkap->client_city, $bkap->client_country ),
			$this->get_description()
		);
		...
		return plugins_url( "woocommerce-booking/includes/common/google-calendar/ical.php?event_date_start=$gmt_start&amp;event_date_end=$gmt_end&amp;current_time=$current_time&amp;summary=$summary&amp;description=$description&amp;event_location=$location" );
	}
	...
	function outlook_cal( $bkap ) {
		...
		$description = str_replace(
			array( 'SITE_NAME', 'CLIENT', 'PRODUCT_NAME', 'PRODUCT_WITH_QTY', 'ORDER_DATE_TIME', 'ORDER_DATE', 'ORDER_NUMBER', 'PRICE', 'PHONE', 'NOTE', 'ADDRESS', 'EMAIL', 'RESOURCE', 'PERSONS', 'ZOOM_MEETING', 'CITY', 'COUNTRY' ),
			array( get_bloginfo( 'name' ), $bkap->client_name, $bkap->product, $bkap->product_with_qty, $bkap->order_date_time, $bkap->order_date, $bkap->id, $bkap->order_total, $bkap->client_phone, $bkap->order_note, $bkap->client_address, $bkap->client_email, $bkap->resource, $bkap->persons, $bkap->zoom_meeting, $bkap->client_city, $bkap->client_country ),
			$this->get_description()
		);
