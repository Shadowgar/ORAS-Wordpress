...
class Notification extends Module_Base {
	...
	protected function check_url_referer( $relation, $val ) {
		...
		if ( isset( $_SERVER['HTTP_REFERER'] ) ) {
			$url = ( ! empty( $val ) ) ? $val : sanitize_text_field( wp_unslash( $_SERVER['HTTP_REFERER'] ) );
			...
			if ( isset( $settings['ex_system_url']['url'] ) && ! empty( $settings['ex_system_url']['url'] ) ) :
				...
				$ref_url = $this->remove_schema_url( rtrim( sanitize_text_field( wp_unslash( $_SERVER['HTTP_REFERER'] ) ), '/\\' ) );
				// end
				if ( ! empty( $val ) ) {
					if ( $val == $ref_url ) {
	...
	protected function get_params_filter( $params ) {
		...
		if ( isset( $_GET[ $params[1] ] ) ) {
			$magic_curly = sanitize_text_field( wp_unslash( $_GET[ $params[1] ] ) );
		} else {
			return '';
		}
		return $magic_curly;
