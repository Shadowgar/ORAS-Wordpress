...
?>
    <div class="mec-liquid-gCalendar mec-gCalendar" id="mec_skin_<?php echo esc_attr($this->id); ?>">
...
<?php }
$button_reset = $this->sf_reset_button ? '<div class="mec-search-reset-button"><button class="button mec-button" id="mec_search_form_' . esc_attr($this->id) . '_reset" type="button">' . esc_html__('Reset', 'mec') . '</button></div>' : '';
$button_type_event = '<div class="mec-event-statuses"><ul class="type-event" style="' . ($this->sf_options['text_search']['type'] == 0 ? 'padding-inline-start: 0px;' : '') . '"><li class="active"><label><input type="radio" name="mec_sf_event_status_' . $this->id . '" class="mec_sf_event_status_' . $this->id . '" value="all" checked="checked" />'
    . esc_html__("All Events", "mec-liq") . '</label></li><li><label><input type="radio" name="mec_sf_event_status_' . $this->id . '" class="mec_sf_event_status_' . $this->id . '" value="EventScheduled" />'
    . esc_html__("Scheduled", "mec-liq") . '</label></li><li><label><input type="radio" name="mec_sf_event_status_' . $this->id . '" class="mec_sf_event_status_' . $this->id . '" value="EventPostponed" />'
    . esc_html__("Postponed", "mec-liq") . '</label></li><li><label><input type="radio" name="mec_sf_event_status_' . $this->id . '" class="mec_sf_event_status_' . $this->id . '" value="EventCancelled" />'
    . esc_html__("Cancelled", "mec-liq") . '</label></li><li><label><input type="radio" name="mec_sf_event_status_' . $this->id . '" class="mec_sf_event_status_' . $this->id . '" value="EventMovedOnline" />'
    . esc_html__("Moved Online", "mec-liq") . '</label></li></ul></div>';
$javascript = '<script>
					...
					click: function() {
						...
						calendar.addEventSource({
							...
							extraParams: {
								...
								locale: "' . esc_js($lang) . '",
								lang: "' . esc_js($lang) . '",
								type_event: jQuery(".mec-event-statuses ul li.active input[name^=\"mec_sf_event_status_' . $this->id . '\"]").val(),
							},
						});
...
$javascript .= '
		...
		const calendarHeaderButton = calendarHeaderLastChild.find(".fc-button-group");
		 jQuery(".fc-button-group").append(\'' . $button_reset . '\');
		 jQuery(".mec-liquid-gCalendar .fc-header-toolbar > div.fc-toolbar-chunk:nth-child(2)").append(\'' . $button_type_event . '\');
		     jQuery(\'.type-event li\').on("click", function () {
        jQuery(this).closest(\'.mec-event-statuses\').find(\'li\').removeClass("active");
        jQuery(this).addClass("active");
    });
		 jQuery(".mec-dropdown-search select").change(function(e) {
						jQuery(".mec-gCalendar-filters").css("display" , "none")
						var eventSource = [];
						eventSource = calendar.getEventSources();
						jQuery.each(eventSource, function (key, value) {
				 ...
				 jQuery(".mec-event-statuses ul li input").change(function(e) {
						jQuery(".mec-gCalendar-filters").css("display" , "none")
						var eventSource = [];
						eventSource = calendar.getEventSources();
						jQuery.each(eventSource, function (key, value) {
				...
				jQuery(".fc-findEvents-button").hide();
...
if (mec_general_calendar_find_event($this->sf_options, 'filter')) :
    $javascript .= '
		...
		jQuery(document).ready(function ($) {
			...
			jQuery(".mec-search-reset-button").on("click", function() {
				reset()
			})
			function reset() {
			 $(\'.mec-event-statuses ul li input[value="all"]\').closest(\'li\').trigger(\'click\');
