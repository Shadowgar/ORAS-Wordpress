 ...
 * @since 1.0.0
 */
class Elementor_EP_SingleOrganizer_Widget extends \Elementor\Widget_Base {
	...
	public function get_name() {
		return 'eventprimeelementorsingleorganizer';
	}
	...
	public function get_title() {
		return __( 'EventPrime Single Event Organizer', 'eventprime-elementor-integration' );
	}
	...
	protected function register_controls() {
        ...
        $organizers_data = $epdb_handler->get_organizers_data();
        $event_organizers_list = array();
        if(isset($organizers_data->terms) && !empty($organizers_data->terms)){
            foreach( $organizers_data->terms as $single_event_organizer ){
                $event_organizers_list[$single_event_organizer->id] = $single_event_organizer->name;
            }
		...
		$this->start_controls_section(
			'content_section',
			[
				'label' => __( 'EventPrime Single Event Organizer', 'eventprime-elementor-integration' ),
        ...
        $this->add_control(
			'event_organizer_id',
			[
				'label' => esc_html__( 'Select Event Organizer', 'eventprime-elementor-integration' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => '',
				'options' => $event_organizers_list,
			]
		);
	...
	protected function render() {
		...
		$atts = array();
        $event_organizer_id = absint( $settings['event_organizer_id'] );
        $atts['id']                     = isset( $event_organizer_id ) ? $event_organizer_id : '';
        if( isset( $atts['id'] ) && ! empty( $atts['id'] ) ){
            ...
            echo $event_controller->load_single_event_organizer($atts);
        }
