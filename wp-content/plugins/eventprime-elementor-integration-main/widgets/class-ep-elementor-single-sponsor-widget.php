 ...
 * @since 1.0.0
 */
class Elementor_EP_SingleSponsor_Widget extends \Elementor\Widget_Base {
	...
	public function get_name() {
		return 'eventprimeelementorsinglesponsor';
	}
	...
	public function get_title() {
		return __( 'EventPrime Single Event Sponsor', 'eventprime-elementor-integration' );
	}
	...
	protected function register_controls() {
		...
		$sponsors_data = $sponsors->get_sponsor_all_data();
        }
        
        $event_sponsors_list = array();
        if(!empty($sponsors_data)){
            foreach( $sponsors_data as $single_event_sponsor ){
                $event_sponsors_list[$single_event_sponsor->ID] = $single_event_sponsor->post_title;
            }
        }
		...
		$this->start_controls_section(
			'content_section',
			[
				'label' => __( 'EventPrime Single Event Performer', 'eventprime-elementor-integration' ),
        ...
        $this->add_control(
			'event_sponsor_id',
			[
				'label' => esc_html__( 'Select Event Sponsor', 'eventprime-elementor-integration' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => '',
				'options' =>  $event_sponsors_list,
			]
		);
	...
	protected function render() {
		...
		$atts = array();
        $event_sponsor_id = absint( $settings['event_sponsor_id'] );
        $atts['id']                     = isset( $event_sponsor_id ) ? $event_sponsor_id : '';
        if( isset( $atts['id'] ) && ! empty( $atts['id'] ) ){
            $atts['event_style']            =   isset( $settings['event_style'] ) ? $settings["event_style"] : 'grid';
            $atts['event_limit']            =   isset( $settings['event_limit'] ) ? $settings['event_limit'] : EM_PAGINATION_LIMIT; 
