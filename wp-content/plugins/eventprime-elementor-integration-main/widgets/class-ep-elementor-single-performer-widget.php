 ...
 * @since 1.0.0
 */
class Elementor_EP_SinglePerformer_Widget extends \Elementor\Widget_Base {
	...
	public function get_name() {
		return 'eventprimeelementorsingleperformer';
	}
	...
	public function get_title() {
		return __( 'EventPrime Single Event Performer', 'eventprime-elementor-integration' );
	}
	...
	protected function register_controls() {
        ...
        $performers_data = $epdb_handler->get_performer_all_data();
        $event_performers_list = array();
        if(!empty($performers_data)){
        foreach( $performers_data as $single_event_performer ){
            $event_performers_list[$single_event_performer->ID] = $single_event_performer->post_title;
        }
        ...
        $this->add_control(
			'event_performer_id',
			[
				'label' => esc_html__( 'Select Event Performer', 'eventprime-elementor-integration' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => '',
				'options' => $event_performers_list,
			]
		);
	...
	protected function render() {
		...
		$atts = array();
        $event_performer_id = absint( $settings['event_performer_id'] );
        $atts['id']                     = isset( $event_performer_id ) ? $event_performer_id : '';
        if( isset( $atts['id'] ) && ! empty( $atts['id'] ) ){
            ...
            echo $event_controller->load_single_performer($atts);
        }
	}
