        ...
        <div class="ep-box-col-12"><?php 
            if( isset( $attendees_data->posts_details->posts ) && ! empty( $attendees_data->posts_details->posts ) ) {
                ...
                <div class="ep-d-flex ep-justify-content-between ep-align-items-center"><?php
                    echo sprintf('%d booking found',$attendees_data->posts_details->found_posts);?>
                    <button type="button" id="ep_attendees_export" class="button-primary ep-btn ep-ar-btn-primary">
                        <?php esc_html_e( 'Export All', 'eventprime-event-advanced-reports' );?>
                    </button>
                </div>
                <table class="form-table ep-table ep-table-striped">
                    ...
                    <tbody>
                            ...
                            <?php foreach($attendees_data->posts_details->posts as $booking){
                ...
                <div class="ep-reports-boooking-load-more">
                    <?php
                    if( isset($attendees_data->posts_details->max_num_pages) && $attendees_data->posts_details->max_num_pages > 1) {?>
                        <div class="ep-report-load-more ep-frontend-loadmore ep-box-w-100 ep-my-4 ep-text-center">
                            <input type="hidden" id="ep-report-attendee-paged" value="1"/>
                            <button type="button" data-max="<?php echo $attendees_data->posts_details->max_num_pages;?>" id="ep-loadmore-report-attendees" class="ep-btn ep-btn-outline-primary button"><span class="ep-spinner ep-spinner-border-sm ep-mr-1"></span><?php esc_html_e( 'Load more', 'eventprime-event-advanced-reports' );?></button>
                        </div><?php
                    }?>
                </div><?php 
            } else{ 
                echo esc_html('No Booking Found.');
            }?>
        </div>
    </div>
</div>